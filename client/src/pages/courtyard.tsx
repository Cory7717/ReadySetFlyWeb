...
const C = {
  ...
  shell:
    "!border-[#ddccb5] !bg-[#fffaf2] !bg-none !text-[#201814] shadow-[0_18px_45px_rgba(74,54,34,0.10)]",
  muted: "!text-[#5f5247]",
  green: "!bg-[#2f5f46] !bg-none !text-white hover:!bg-[#274d39]",
  accent: "!bg-[#b98435] !bg-none !text-white hover:!bg-[#9f6f2b]",
  field:
    "!border-[#cdbda8] !bg-white !text-[#201814] placeholder:!text-[#76695d]",
  outline:
    "!border-[#cdbda8] !bg-white !bg-none !text-[#201814] hover:!bg-[#f8efe2]",
...
const DEFAULT_TOOL_KEYS: ToolKey[] = [
  "schedule",
  "tips",
  "opsreport",
  "dosreporting",
...
function userHasTipsAccess(user: CourtyardUser) {
  return (
    user.canAccessTips ??
    (user.isAdmin || (user.scheduleRoles || []).some(isBistroRole))
  );
}
...
function CourtyardLogin({
  onDone,
}: {
  onDone: (user?: CourtyardUser) => void;
}) {
  ...
  const login = useMutation({
    mutationFn: async () => {
      if (
        mode === "register" &&
        access.tips &&
        !form.rolesJson.some(isBistroRole)
      ) {
  ...
  const resetPassword = useMutation({
    mutationFn: () =>
      apiRequest("POST", "/api/tips/auth/password-reset-request", {
        email: form.email,
      }),
    onSuccess: () =>
      toast({
        title: "Check your email",
        description: "If that account exists, a temporary password was sent.",
      }),
    onError: (error: Error) =>
      toast({
  ...
  const toggleRole = (role: string) =>
    setForm((current) => {
      ...
      return {
        ...current,
        department:
          current.department === "Above Property"
            ? "Managers"
            : current.department,
        rolesJson: current.rolesJson.includes(role)
          ? current.rolesJson.filter((item) => item !== role)
          : [
              ...current.rolesJson.filter((item) => item !== "Above Property"),
              role,
            ],
      };
    });
    ...
    <div className={`min-h-screen ${C.page}`}>
      <main className="mx-auto flex min-h-screen max-w-lg items-center px-4 py-8">
        <Card className={`w-full ${C.shell}`}>
          <CardHeader>
            <div className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8a6b3f]">
              Courtyard Austin Lakeline
            </div>
            <CardTitle className="text-3xl">Associate Portal</CardTitle>
            <CardDescription className={C.muted}>
              {mode === "login"
                ? "Sign in with your Courtyard account."
                : "Create your Courtyard account for schedule access, tips access, or both."}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {mode === "register" && (
              <>
                <div className="grid gap-3 sm:grid-cols-2">
                  <button
                    type="button"
                    className={`rounded-lg border p-3 text-left text-sm ${access.schedule ? "border-[#2f5f46] bg-[#e8f3ec]" : "border-[#cdbda8] bg-white"}`}
                    onClick={() =>
                      setAccess({ ...access, schedule: !access.schedule })
                    }
                  >
                    <div className="font-semibold">Schedule</div>
                    <div className="text-[#5f5247]">
                      View schedules and submit requests.
                    </div>
                  </button>
                  <button
                    type="button"
                    className={`rounded-lg border p-3 text-left text-sm ${access.tips ? "border-[#b98435] bg-[#fff3dd]" : "border-[#cdbda8] bg-white"}`}
                    onClick={() => setAccess({ ...access, tips: !access.tips })}
                  >
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <Input
                      className={C.field}
                      value={form.employeeDisplayName}
                      onChange={(event) =>
                        setForm({
                          ...form,
                          employeeDisplayName: event.target.value,
                        })
                      }
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-2">
                    {SCHEDULE_ROLES.map((role) => (
                      <Button
                        ...
                        variant="outline"
                        className={
                          form.rolesJson.includes(role) ? C.green : C.outline
                        }
                  ...
                  {access.tips && !form.rolesJson.some(isBistroRole) && (
                    <p className="mt-1 text-sm text-amber-800">
                      Select Bistro AM, Bistro PM, or Breakfast to request Tips
                      access.
                    </p>
                  )}
                </div>
            ...
            {mode === "login" && (
              <Button
                variant="ghost"
                className="w-full text-[#2f5f46]"
                disabled={!form.email || resetPassword.isPending}
...
function CourtyardPasswordChange({
  onDone,
}: {
  onDone: (user?: CourtyardUser) => void;
}) {
      ...
      <main className="mx-auto flex min-h-screen max-w-lg items-center px-4 py-8">
        <Card className={`w-full ${C.shell}`}>
          <CardHeader>
            ...
            <CardDescription className={C.muted}>
              Confirm the temporary password from your email, then create your
              permanent password.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
...
function CourtyardToolAccessAdmin() {
  ...
  const updateAccess = useMutation({
    mutationFn: async ({
      ...
      const response = await apiRequest(
        "PATCH",
        `/api/tips/admin/users/${userId}/tool-access`,
        { tool, enabled },
      );
      return response.json();
    },
  ...
  const sendPasswordReset = useMutation({
    mutationFn: async (userId: string) => {
      const response = await apiRequest(
        "POST",
        `/api/tips/admin/users/${userId}/password-reset-email`,
        {},
      );
      return response.json();
    },
    onSuccess: (_data, userId) => {
      const associate = accessUsers.data?.users.find(
        (item) => item.id === userId,
      );
      queryClient.invalidateQueries({
        queryKey: ["/api/tips/admin/tool-access-users"],
  ...
  const users = (accessUsers.data?.users || []).filter((item) => {
    ...
    return `${item.employeeDisplayName} ${item.email} ${item.position || ""}`
      .toLowerCase()
      .includes(query);
  });
  ...
  const tools = accessUsers.data?.tools?.length
    ? accessUsers.data.tools
    : DEFAULT_TOOL_KEYS;
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <Settings2 className="h-5 w-5 text-[#8a6b3f]" /> Tool Access
            </CardTitle>
            <CardDescription className={C.muted}>
              Grant or withdraw access to Courtyard tools without changing an
              associate's job role.
            </CardDescription>
          </div>
            ...
            <table className="w-full min-w-[1280px] border-collapse text-sm">
              <thead>
                <tr className="border-b border-[#d7c8b5] text-left text-xs uppercase tracking-[0.12em] text-[#6b5f54]">
                  ...
                  {tools.map((tool) => (
                    <th key={tool} className="px-3 py-2 text-center">
                      {toolLabels[tool]}
                    </th>
                  ))}
              ...
              <tbody>
                {users.map((item) => (
                  <tr
                    key={item.id}
                    className="border-b border-[#eadfce] last:border-0"
                  >
                    <td className="py-3 pr-3">
                      <div className="font-semibold text-[#201814]">
                    ...
                    <td className="px-3 py-3">
                      <Badge
                        variant="outline"
                        className="border-[#cdbda8] bg-white text-[#201814]"
                      >
                        {item.role.replace("_", " ")}
                      </Badge>
                    </td>
                      ...
                      <td key={tool} className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <Switch
                            ...
                            disabled={
                              item.isSuperAdmin || updateAccess.isPending
                            }
                          ...
                          />
                          <span className="w-16 text-left text-xs text-[#5f5247]">
                            {toolEnabled(item, tool) ? "Enabled" : "Off"}
                          </span>
                        </div>
                      </td>
                    ))}
                    <td className="px-3 py-3 text-center">
                      <Button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              `Send a temporary password to ${item.employeeDisplayName} at ${item.email}? Their current password will stop working immediately.`,
                            )
                          ) {
  ...
  if (!auth.data?.user) {
    return (
      <CourtyardLogin
        onDone={(user) => {
          if (user)
            queryClient.setQueryData(["/api/schedule/auth/me", "courtyard"], {
              user,
            });
          queryClient.invalidateQueries({
            queryKey: ["/api/schedule/auth/me", "courtyard"],
  ...
  const tools = [
    {
      ...
      description:
        "Submit and track time-off, availability, or preferred-shift requests. Available to every associate.",
    ...
    },
    {
      ...
      description: user.isAdmin
        ? "Build schedules, review requests, complete department sections, and publish final schedules."
        : "View your published schedule and submit time-off or availability requests.",
    ...
    },
    {
      ...
      title: "Bistro Tips",
      description:
        "Open the Bistro tip reporting page. Associates enter the 5 digit team PIN before entering tip reports.",
    ...
    },
    {
      ...
      title: "Ops Report",
      description:
        "Open the weekly operations report workspace when you have access to that tool.",
    ...
    },
    {
      ...
      title: "Comptroller",
      description:
        "Calculate monthly hotel tax payable from STAY reports and review Texas, Austin, TPID, and exemption compliance checks.",
    ...
    },
    {
      ...
      title: "DOS Reporting",
      description:
        "Complete weekly sales activity, monthly pipeline, top accounts, production, and future business reports.",
    ...
    },
    {
      ...
      description:
        "Open saved incident reports, submit a new report, download PDFs, and resend manager email copies.",
    ...
    },
    {
      ...
      title: "Bank Deposit",
      description:
        "Track daily system drop counts against actual deposits with monthly variance totals.",
      ...
      ? [
          {
            ...
            title: "Budget",
            description:
              "View monthly labor, expense, revenue, forecast, and department checkbook budgets.",
            action: "Open Budget",
            tone: C.green,
            disabled: false,
          },
      ...
      : []),
    {
      ...
      description:
        "Upload Marriott MINT account production reports, review historical group business, and identify accounts that may need recovery outreach.",
    ...
    <div className={`min-h-screen ${C.page}`}>
      <header className="border-b border-[#deceba] bg-[#fffaf2]/95 px-4 py-4 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {user.isSuperAdmin && (
              <Badge className="bg-[#2f5f46] text-white">
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-4 py-6">
          ...
          <CardContent className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="text-xl font-semibold">
                {user.employeeDisplayName || user.email}
              </div>
              <div className="text-sm text-[#5f5247]">{user.email}</div>
            </div>
            {user.mustChangePassword && (
              <Button asChild className={C.accent}>
                <Link href="/tips">Set your password</Link>
              </Button>
            )}
          </CardContent>
        </Card>
          ...
          {tools.map((tool) => {
            ...
            return (
              <Card key={tool.href} className={C.shell}>
                ...
                <CardContent>
                  {tool.disabled ? (
                    <Button className={`w-full ${C.outline}`} disabled>
                      Access not enabled
                    </Button>
                  ) : (
        ...
        <Card className={C.shell}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <DoorOpen className="h-5 w-5 text-[#8a6b3f]" /> Quick Guidance
            </CardTitle>
          </CardHeader>
          <CardContent className="grid gap-3 text-sm text-[#5f5247] md:grid-cols-3">
            <div>
              Use <strong className="text-[#201814]">Schedule</strong> for
              posted schedules and time-off requests.
            </div>
            <div>
              Use <strong className="text-[#201814]">Bistro Tips</strong> only
              if you report Bistro credit-card tips.
            </div>
            <div>
              Managers may see additional controls inside each tool based on
              their assigned role.
            </div>
          </CardContent>
        </Card>
