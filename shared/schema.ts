...
export const tipGridDaySummaries = pgTable("tip_grid_day_summaries", {
  ...
  beerSales: numeric("beer_sales", { precision: 10, scale: 2 }).notNull().default("0"),
  liquorSales: numeric("liquor_sales", { precision: 10, scale: 2 }).notNull().default("0"),
  foodSales: numeric("food_sales", { precision: 10, scale: 2 }).notNull().default("0"),
  wineSales: numeric("wine_sales", { precision: 10, scale: 2 }).notNull().default("0"),
  notes: text("notes"),
...
export const tipBanquetReports = pgTable("tip_banquet_reports", {
  ...
  grossSales: numeric("gross_sales", { precision: 10, scale: 2 }).notNull().default("0"),
  serviceRate: numeric("service_rate", { precision: 5, scale: 4 }).notNull().default("0.2100"),
  banquetTips: numeric("banquet_tips", { precision: 10, scale: 2 }).notNull().default("0"),
  assignedAssociatesJson: jsonb("assigned_associates_json").$type<Array<{ userId: string; displayName: string; splitAmount: string }>>().notNull().default(sql`'[]'::jsonb`),
  notes: text("notes"),
...
export const scheduleForecastDays = pgTable("schedule_forecast_days", {
  ...
  actualRoomRevenue: numeric("actual_room_revenue", { precision: 12, scale: 2 }),
...
export const courtyardComptrollerReports = pgTable("courtyard_comptroller_reports", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  propertyId: text("property_id").notNull().default("courtyard-austin-lakeline"),
  reportMonth: text("report_month").notNull(),
  payloadJson: jsonb("payload_json").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
  ...
  uniqueIndex("idx_courtyard_comptroller_report_period").on(table.propertyId, table.reportMonth),
...
export const logbookEntries = pgTable("logbook_entries", {
  ...
  solo: decimal("solo", { precision: 6, scale: 2 }).default(sql`0`).notNull(),
  crossCountry: decimal("cross_country", { precision: 6, scale: 2 }).default(sql`0`).notNull(),
...
export const flyingClubs = pgTable("flying_clubs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ownerUserId: varchar("owner_user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  slug: text("slug").notNull(),
...
export const flyingClubMembers = pgTable("flying_club_members", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
export const flyingClubAircraft = pgTable("flying_club_aircraft", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  clubId: varchar("club_id").notNull().references(() => flyingClubs.id, { onDelete: "cascade" }),
  aircraftListingId: varchar("aircraft_listing_id").references(() => aircraftListings.id, { onDelete: "set null" }),
  displayName: text("display_name").notNull(),
  tailNumber: text("tail_number"),
...
export const flyingClubReservations = pgTable("flying_club_reservations", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  clubId: varchar("club_id").notNull().references(() => flyingClubs.id, { onDelete: "cascade" }),
...
export const flyingClubAnnouncements = pgTable("flying_club_announcements", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  clubId: varchar("club_id").notNull().references(() => flyingClubs.id, { onDelete: "cascade" }),
  ...
  index("idx_flying_club_announcements_club").on(table.clubId),
]);

export const flyingClubDocuments = pgTable("flying_club_documents", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  clubId: varchar("club_id").notNull().references(() => flyingClubs.id, { onDelete: "cascade" }),
  uploadedByUserId: varchar("uploaded_by_user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  title: text("title").notNull(),
...
export const userSettings = pgTable("user_settings", {
  ...
  flightServiceProfile: jsonb("flight_service_profile").$type<Record<string, unknown> | null>(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
...
export const flightPlans = pgTable("flight_plans", {
  ...
  filingProviderMessages: jsonb("filing_provider_messages").$type<Array<Record<string, unknown>>>().default(sql`'[]'::jsonb`),
...
export const aircraftProfiles = pgTable("aircraft_profiles", {
  ...
  fuelBurnDefaultGph: decimal("fuel_burn_default_gph", { precision: 6, scale: 2 }),
  notes: text("notes"),
  ...
  filingPerformanceCategoryDefault: text("filing_performance_category_default"),
  ...
  index("idx_aircraft_profiles_user_default").on(table.userId, table.isDefault),
]);
...
export const insertFlyingClubMemberSchema = createInsertSchema(flyingClubMembers).omit({
  id: true,
  clubId: true,
  createdAt: true,
  updatedAt: true,
...
export const insertFlyingClubReservationSchema = createInsertSchema(flyingClubReservations).omit({
  id: true,
  clubId: true,
  createdAt: true,
  updatedAt: true,
...
export const insertFlyingClubAnnouncementSchema = createInsertSchema(flyingClubAnnouncements).omit({
  id: true,
  clubId: true,
  authorUserId: true,
  createdAt: true,
...
export const insertFlyingClubDocumentSchema = createInsertSchema(flyingClubDocuments).omit({
  id: true,
  clubId: true,
  uploadedByUserId: true,
  createdAt: true,
...
export const insertAircraftProfileSchema = createInsertSchema(aircraftProfiles).omit({
  ...
  engineCountOverride: z.coerce.number().int().min(0).max(12).optional().nullable(),
  ...
  serviceCeilingFt: z.coerce.number().int().min(1000).max(60000).optional().nullable(),
  ...
  filingEmergencyEquipmentDefault: z.string().trim().max(100).optional().nullable(),
  filingTransponderDefault: z.string().trim().max(50).optional().nullable(),
  filingPerformanceCategoryDefault: z.string().trim().max(50).optional().nullable(),
  ...
  filingAltitudePreferenceDefault: z.string().trim().max(20).optional().nullable(),
  filingEnduranceMinutesDefault: z.coerce.number().int().min(1).max(1440).optional().nullable(),
});
...
export const insertBannerAdOrderSchema = createInsertSchema(bannerAdOrders).omit({
  ...
  title: z.string().min(1, "Title is required"),
  description: z.union([z.string().max(240), z.literal(""), z.null()]).optional(),
  adCopy: z.union([z.string().max(800), z.literal(""), z.null()]).optional(),
  imageUrl: z.union([z.string().url("Image URL must be valid"), z.literal(""), z.null()]).optional(),
  videoUrl: z.union([z.string().url("Video URL must be valid"), z.literal(""), z.null()]).optional(),
...
export const insertBannerAdSchema = createInsertSchema(bannerAds).omit({
  ...
  description: z.union([z.string().max(240), z.literal(""), z.null()]).optional(),
  adCopy: z.union([z.string().max(800), z.literal(""), z.null()]).optional(),
  instagramUrl: z.union([z.string().max(240), z.literal(""), z.null()]).optional(),
  facebookUrl: z.union([z.string().max(240), z.literal(""), z.null()]).optional(),
});
...
export type InsertFlyingClubReservation = z.infer<typeof insertFlyingClubReservationSchema>;
export type FlyingClubAnnouncement = typeof flyingClubAnnouncements.$inferSelect;
