...
const sensitiveKey = /(^|\.)(authorization|password|passwd|secret|api.?key|access.?token|database.?url|webhook.?url|provider.?plan.?id|internal.?plan.?id|user.?id|request.?id|event.?hash|stack|stack.?trace)$/i;
...
export function findSensitiveValidationReportValue(value: unknown, path = "$"): { path: string; reason: string } | null {
  ...
  if (value && typeof value === "object") {
    for (const [key, nested] of Object.entries(value as Record<string, unknown>)) {
      ...
      if (sensitiveKey.test(nestedPath) && nested != null && String(nested).trim()) return { path: nestedPath, reason: "sensitive field" };
...
export function validatePublicValidationReport(value: unknown) {
  ...
  if (!parsed.success) return { ok: false as const, error: "The report does not match schema version 1.0.", details: parsed.error.flatten() };
