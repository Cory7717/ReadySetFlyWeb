...
export function registerCourtyardSalesIntelligenceRoutes(app: Express) {
  ...
  router.post("/meeting-calendar/events", async (req:any,res,next)=>{try{const hotelId=String(req.body.hotelId||"");if(!hasHotel(req,hotelId))return res.status(403).json({error:"Property access required."});const required=["spaceId","groupName","eventName","eventDate","setupStartTime","guestStartTime","guestEndTime","breakdownEndTime"];if(required.some(k=>!String(req.body[k]||"").trim()))return res.status(400).json({error:"Group, event, date, space, and all operational times are required."});if(!(req.body.setupStartTime<=req.body.guestStartTime&&req.body.guestStartTime<req.body.guestEndTime&&req.body.guestEndTime<=req.body.breakdownEndTime))return res.status(400).json({error:"Times must follow setup start, guest start, guest end, then breakdown end."});const holdExpiresAt=req.body.holdExpiresAt?new Date(req.body.holdExpiresAt):null;if(holdExpiresAt&&Number.isNaN(holdExpiresAt.getTime()))return res.status(400).json({error:"Enter a valid courtesy-hold expiration date and time."});const active=(await db.select().from(courtyardMeetingEvents).where(and(eq(courtyardMeetingEvents.spaceId,req.body.spaceId),eq(courtyardMeetingEvents.eventDate,req.body.eventDate)))).filter(x=>!["cancelled","completed","expired"].includes(x.status));const conflict=active.find(x=>req.body.setupStartTime<x.breakdownEndTime&&x.setupStartTime<req.body.breakdownEndTime);if(conflict&&(!canManageMeetingCalendar(req)||!String(req.body.conflictOverrideReason||"").trim()))return res.status(409).json({error:`The space is occupied by ${conflict.groupName} from ${conflict.setupStartTime.slice(0,5)} to ${conflict.breakdownEndTime.slice(0,5)}. An override reason is required.`,code:"MEETING_SPACE_CONFLICT"});if(req.body.status==="definite"&&!canManageMeetingCalendar(req))return res.status(403).json({error:"Calendar PIN access is required to mark an event definite."});const [event]=await db.insert(courtyardMeetingEvents).values({...meetingEventWriteValues(req.body,holdExpiresAt),createdByUserId:req.salesUser.id,updatedByUserId:req.salesUser.id}).returning();res.status(201).json({event});}catch(e){next(e);}});
  router.patch("/meeting-calendar/events/:id",async(req:any,res,next)=>{try{const [existing]=await db.select().from(courtyardMeetingEvents).where(eq(courtyardMeetingEvents.id,req.params.id)).limit(1);if(!existing||!hasHotel(req,existing.hotelId))return res.status(404).json({error:"Event not found."});if(req.body.status==="definite"&&!canManageMeetingCalendar(req))return res.status(403).json({error:"Calendar PIN access is required to mark an event definite."});const holdExpiresAt=req.body.holdExpiresAt?new Date(req.body.holdExpiresAt):null;if(holdExpiresAt&&Number.isNaN(holdExpiresAt.getTime()))return res.status(400).json({error:"Enter a valid courtesy-hold expiration date and time."});const [event]=await db.update(courtyardMeetingEvents).set({...meetingEventWriteValues(req.body,holdExpiresAt),updatedByUserId:req.salesUser.id,updatedAt:new Date()}).where(eq(courtyardMeetingEvents.id,existing.id)).returning();res.json({event});}catch(e){next(e);}});
