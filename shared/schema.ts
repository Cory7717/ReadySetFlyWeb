...
export const courtyardMeetingEvents = pgTable("courtyard_meeting_events", {
  ...
  setupOrientation: text("setup_orientation").notNull().default("lengthwise"), setupLayoutJson: jsonb("setup_layout_json").$type<Array<{ id: string; x: number; y: number }>>().notNull().default(sql`'[]'::jsonb`),
