...
function MeetingSetupDiagram({ meetingRoom, roomSetup, attendance, banquetChairsPerTable = 8, orientation = "lengthwise", layout = [], onLayoutChange }: { meetingRoom: string; roomSetup: string; attendance: number | string; banquetChairsPerTable?: number | string; orientation?: string; layout?: any[]; onLayoutChange?: (layout: any[]) => void }) {
  ...
  const innerLeft=55,innerTop=72,innerWidth=490,innerHeight=220;
  const position = (id: string, x: number, y: number) => { const saved = layout.find((item:any) => item.id === id); return saved ? {x:innerLeft+Number(saved.x)*innerWidth,y:innerTop+Number(saved.y)*innerHeight} : {x,y}; };
  const drag = (id: string) => onLayoutChange ? { style: { cursor: "grab" }, onPointerDown: (event:any) => { event.preventDefault(); const item=layout.find((candidate:any)=>candidate.id===id);if(item?.type&&!['presentation','entry'].includes(item.type))setSelectedLayoutId(id);event.currentTarget.setPointerCapture(event.pointerId); }, onPointerMove: (event:any) => { if (!event.currentTarget.hasPointerCapture(event.pointerId)) return; const svg = event.currentTarget.ownerSVGElement, bounds = svg.getBoundingClientRect(); const x = Math.max(0, Math.min(1, ((event.clientX-bounds.left)/bounds.width*600-innerLeft)/innerWidth)), y = Math.max(0, Math.min(1, ((event.clientY-bounds.top)/bounds.height*350-innerTop)/innerHeight)); const existing=layout.find((item:any)=>item.id===id)||{id}; onLayoutChange([...layout.filter((item:any)=>item.id!==id),{...existing,id,x,y}]); }, onPointerUp: (event:any) => { if(event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); }, onPointerCancel: (event:any) => { if(event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); } } : {};
  ...
  if (roomSetup === "banquet") {
    ...
    elements.push(<path key="u" d="M155 90 L155 250 L445 250 L445 90" fill="none" stroke="#7b684f" strokeWidth="20"/>);
    for(let i=0;i<leftCount;i++){const fraction=(i+1)/(leftCount+1);elements.push(<circle key={`left-${i}`} cx={132} cy={90+fraction*160} r="5" fill="#315f86"/>);}
    for(let i=0;i<rightCount;i++){const fraction=(i+1)/(rightCount+1);elements.push(<circle key={`right-${i}`} cx={468} cy={90+fraction*160} r="5" fill="#315f86"/>);}
    for(let i=0;i<bottomCount;i++){const fraction=(i+1)/(bottomCount+1);elements.push(<circle key={`bottom-${i}`} cx={155+fraction*290} cy={273} r="5" fill="#315f86"/>);}
