...
export default function CourtyardMeetingCalendar() {
  ...
  const importContract = useMutation<any, any, boolean>({
    ...
    onSuccess: (result: any) => { setContractOpen(false); setContractFile(null); setContractPreview(null); setContractMergeMessage(""); qc.invalidateQueries({ queryKey: ["meeting-calendar"] }); toast({ title: result.count ? "Linked group booking imported" : "Group Rooms booking imported", description: result.count ? `Saved the room block and ${result.count} meeting-space date${result.count === 1 ? "" : "s"}.` : "Saved the rooms-only contract without creating a meeting-space event." }); },
