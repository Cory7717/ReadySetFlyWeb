...
test("flight-planner lifecycle controls are not hard-suppressed", () => {
  ...
  assert.doesNotMatch(combinedUi, /false\s*&&[\s\S]{0,180}(Amend|Activate|Cancel|Close|Sync|Provider updates|Mark update reviewed)/i);
  assert.doesNotMatch(combinedUi, /Boolean\(false\)[\s\S]{0,180}(Amend|Activate|Cancel|Close|Sync|Provider updates|Mark update reviewed)/i);
  for (const label of [
    "File Flight Plan",
    "Amend",
    "Activate",
