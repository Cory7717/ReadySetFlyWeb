...
function FlightDemoDataRail({
  ...
  ownship: { speedKts: number; heading: number };
  remainingRouteNm: number;
  selectedTrafficTarget: DemoTrafficTarget | null;
}) {
  const cells = [
    { label: "Mode", value: phaseLabel },
    { label: "Next", value: nextWaypoint ? `${nextWaypoint}${nextWaypointDistanceNm != null ? ` ${nextWaypointDistanceNm.toFixed(1)} NM` : ""}` : "Route active" },
    { label: "GS", value: `${Math.round(ownship.speedKts)} KT` },
    { label: "TRK", value: `${Math.round(ownship.heading).toString().padStart(3, "0")}°` },
    {
      label: "Traffic",
      value: selectedTrafficTarget
        ? `${selectedTrafficTarget.callsign} ${selectedTrafficTarget.distanceNm.toFixed(1)} NM`
        : `${remainingRouteNm.toFixed(0)} NM remain`,
    },
  ];

  return (
    <div className="absolute inset-x-3 top-3 z-30 hidden rounded-[18px] border border-[#24364D] bg-[#07111C]/86 px-3 py-2 shadow-[0_12px_30px_rgba(0,0,0,0.25)] backdrop-blur md:block">
      <div className="grid grid-cols-5 gap-3">
        {cells.map((cell, index) => (
          <div key={cell.label} className={index < cells.length - 1 ? "border-r border-[#1B2A3C] pr-3" : ""}>
            <div className="text-[9px] font-semibold uppercase tracking-[0.22em] text-[#76A8D4]">{cell.label}</div>
            <div className="mt-1 font-mono text-sm text-[#E8EDF4]">{cell.value}</div>
          </div>
        ))}
      </div>
    </div>
...
function FlightDemoInsetRadar({
  ...
  const projectedPoints = routePoints
    .map((point) => {
      ...
      return {
        icao: point.icao,
        x: 50 + (rightNm / rangeNm) * 34,
        y: 50 - (forwardNm / rangeNm) * 34,
      };
    })
    .filter((point) => point.x >= 8 && point.x <= 92 && point.y >= 8 && point.y <= 92);

  const projectedTraffic = trafficTargets
    .map((target) => {
      const { forwardNm, rightNm } = projectPointRelativeToOwnship(ownship, {
        latitude: target.lat,
        longitude: target.lon,
      });
      return {
  ...
  return (
    <div className="absolute bottom-[116px] left-6 z-20 hidden w-[118px] rounded-[20px] border border-[#1E2D42] bg-[#07111C]/88 p-2.5 shadow-[0_12px_30px_rgba(0,0,0,0.22)] backdrop-blur lg:block">
      ...
      <svg viewBox="0 0 100 100" className="mt-2 h-[90px] w-full">
        ...
        <path d="M50 17 L53 24 L50 22 L47 24 Z" fill="#E8EDF4" />
        {projectedPoints.map((point) => (
          <g key={`inset-${point.icao}`}>
            <circle
              cx={point.x}
              cy={point.y}
              r={point.icao === nextWaypoint ? 3.2 : 2.1}
              fill={point.icao === nextWaypoint ? "#00D4A0" : "#4A9FD4"}
            />
            <text x={point.x + 3.6} y={point.y - 1.5} fill="#D7E4F2" fontSize="5.1" letterSpacing="0.08">
              {point.icao}
            </text>
          </g>
        ))}
        {projectedTraffic.map((target) => (
          <rect
            key={`traffic-${target.id}`}
            x={target.x - 2.4}
            y={target.y - 2.4}
            width="4.8"
            height="4.8"
            transform={`rotate(45 ${target.x} ${target.y})`}
            fill={target.threatLevel === "immediate" ? "#E8453C" : target.threatLevel === "advisory" ? "#F5A623" : "#E8EDF4"}
            stroke="#0A0E14"
            strokeWidth="0.8"
          />
        ))}
        <path d="M50 50 L50 44 L53 51 L50 49 L47 51 Z" fill="#E8EDF4" />
      </svg>
    </div>
  );
}
...
function AirportSurfacePreview({
  ...
  const [surfaceView, setSurfaceView] = useState<"schematic" | "live">("live");

  useEffect(() => {
    setSurfaceView(liveDiagram ? "live" : "schematic");
  }, [liveDiagram, preview?.airportIcao, preview?.runwayId]);
...
function FlightDemoVisionSurface({
  ...
  const isTaxiOut = flightPhase === "surface-departure";
  const isDeparture = flightPhase === "departure";
  const isEnroute = flightPhase === "enroute";
  const isArrival = flightPhase === "arrival";
  const isTaxiIn = flightPhase === "surface-arrival";
  ...
  const bankDeg = clamp(routeCue * 1.15, isArrival ? -18 : -22, isArrival ? 18 : 22);
  const pitchDeg = clamp(verticalSpeedFpm / 140, isArrival ? -6 : -7, isDeparture ? 9 : 8);
  ...
  const gateCenters = isEnroute
    ? [routeCue * 0.22, routeCue * 0.45, routeCue * 0.72, routeCue]
    : isDeparture
      ? [routeCue * 0.12, routeCue * 0.28, routeCue * 0.46]
      : isArrival
        ? [routeCue * 0.15, routeCue * 0.3]
        : [];
  ...
  const phaseSupport =
    isTaxiOut
      ? "Hold-short, runway entry, and initial roll guidance."
      : isDeparture
        ? "Runway centerline and climb corridor are active."
        : isArrival
          ? "Runway box and centerline guidance are staged for landing."
          : isTaxiIn
            ? "Landing rollout is complete and taxi-in is staged."
            : "Traffic, terrain, and route tunnel remain primary.";
  ...
  const runwayRibbonLeft = 50 + routeCue * 0.18;
  ...
  return (
    <div className="relative h-[456px] overflow-hidden rounded-[24px] border border-slate-800 bg-[#060A10]">
      <FlightDemoDataRail
        ...
        ownship={ownship}
        remainingRouteNm={remainingRouteNm}
      ...
      >
        ...
        {terrainBands.length ? (
          <svg className="absolute inset-x-0 bottom-0 h-[54%] w-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            {terrainBands.map((band, index) => (
              <path key={`terrain-band-${index}`} d={band.d} fill={band.fill} />
            ))}
          </svg>
        ) : null}
        {obstacleColumn ? (
          <div
            className="absolute w-[3px] rounded-full bg-[#E8453C]/85 shadow-[0_0_18px_rgba(232,69,60,0.35)]"
            style={{
              left: `${obstacleColumn.leftPct}%`,
              top: `${obstacleColumn.topPct}%`,
              height: `${obstacleColumn.heightPct}%`,
            }}
          />
        ) : null}
      ...
      {showRunwayGuidance ? (
        <div
          className="absolute bottom-0 z-10 -translate-x-1/2 overflow-hidden rounded-t-[22px] border border-white/15 bg-[linear-gradient(180deg,rgba(28,33,42,0.2)_0%,rgba(24,29,36,0.72)_18%,rgba(18,22,28,0.96)_100%)]"
          style={{
            left: `${runwayRibbonLeft}%`,
            width: `${runwayRibbonWidth}%`,
            top: `${runwayRibbonTop}%`,
      ...
      {terrainState ? (
        <div className={terrainCardClass}>
      ...
      {selectedTrafficTarget ? (
        <div className={trafficCardClass}>
          <div className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#7A9BB8]">Traffic</div>
          <div className="mt-1 text-sm font-semibold text-[#E8EDF4]">
...
export default function SyntheticVisionPage() {
  ...
  const [speed, setSpeed] = useState<DemoSpeed>("1x");
