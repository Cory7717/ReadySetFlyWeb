...
function MeetingSetupDiagram({ meetingRoom, roomSetup, attendance, banquetChairsPerTable = 8, orientation = "lengthwise", layout = [], onLayoutChange }: { meetingRoom: string; roomSetup: string; attendance: number | string; banquetChairsPerTable?: number | string; orientation?: string; layout?: any[]; onLayoutChange?: (layout: any[]) => void }) {
  ...
  // Preserve the real room aspect ratio: length runs left-to-right and width runs top-to-bottom.
  const feetToCanvas = Math.min(530 / room.lengthFeet, 420 / room.widthFeet);
  const roomWidth = room.lengthFeet * feetToCanvas, roomHeight = room.widthFeet * feetToCanvas, roomLeft = (600-roomWidth)/2, roomTop = (500-roomHeight)/2;
  ...
  const innerLeft=roomLeft+roundTableRadius+9,innerTop=roomTop+roundTableRadius+9,innerWidth=Math.max(1,roomWidth-(roundTableRadius+9)*2),innerHeight=Math.max(1,roomHeight-(roundTableRadius+9)*2);
  const position = (id: string, x: number, y: number) => { const saved = layout.find((item:any) => item.id === id); if(saved)return{x:innerLeft+Number(saved.x)*innerWidth,y:innerTop+Number(saved.y)*innerHeight};return{x:innerLeft+Math.max(0,Math.min(1,(x-55)/490))*innerWidth,y:innerTop+Math.max(0,Math.min(1,(y-72)/220))*innerHeight}; };
  const drag = (id: string) => onLayoutChange ? { style: { cursor: "grab" }, onPointerDown: (event:any) => event.currentTarget.setPointerCapture(event.pointerId), onPointerMove: (event:any) => { if (!event.currentTarget.hasPointerCapture(event.pointerId)) return; const svg = event.currentTarget.ownerSVGElement, bounds = svg.getBoundingClientRect(); const x = Math.max(0, Math.min(1, ((event.clientX-bounds.left)/bounds.width*600-innerLeft)/innerWidth)), y = Math.max(0, Math.min(1, ((event.clientY-bounds.top)/bounds.height*500-innerTop)/innerHeight)); const existing=layout.find((item:any)=>item.id===id)||{id}; onLayoutChange([...layout.filter((item:any)=>item.id!==id),{...existing,id,x,y}]); }, onPointerUp: (event:any) => event.currentTarget.releasePointerCapture(event.pointerId) } : {};
