...
function SalesTransitionHub({ hotelId, isAdmin }: { hotelId: string; isAdmin: boolean }) {
  ...
  const [setup, setSetup] = useState({ title: "Director of Sales Transition", departureDate: "2026-08-28", departingUserName: "", summary: "" });
  ...
  const [item, setItem] = useState(emptyItem); const [file, setFile] = useState<File | null>(null); const [fileCategory, setFileCategory] = useState("other");
  ...
  return <div className="space-y-4">
    ...
    <Card className={C.shell}><CardHeader><CardTitle>Document archive</CardTitle><CardDescription>Contracts, reports, SOPs, templates, and collateral · 10 MB maximum.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="flex flex-wrap gap-2"><Input className="max-w-sm" type="file" onChange={e=>setFile(e.target.files?.[0]||null)}/><Input className="max-w-48" placeholder="Category" value={fileCategory} onChange={e=>setFileCategory(e.target.value)}/><Button className={C.green} disabled={!file} onClick={async()=>{if(!file)return;const f=new FormData();f.append("file",file);f.append("category",fileCategory);await json(`/api/courtyard/sales-intelligence/transitions/${t.id}/documents`,{method:"POST",body:f});setFile(null);refresh()}}>Archive file</Button></div>{(query.data.documents||[]).map((x:any)=><div key={x.id} className="flex justify-between rounded border bg-white p-2 text-sm"><span>{x.filename} · {x.category} · {Math.ceil(x.sizeBytes/1024)} KB</span><Button asChild size="sm" variant="outline"><a href={apiUrl(`/api/courtyard/sales-intelligence/transitions/${t.id}/documents/${x.id}`)}>Download</a></Button></div>)}</CardContent></Card>
    <Card className={C.shell}><CardHeader><CardTitle>Secure sharing</CardTitle><CardDescription>Links expire and can be revoked. Confidential and credential fields are always excluded.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="grid gap-2 md:grid-cols-4"><Input placeholder="Recipient name" value={share.recipientName} onChange={e=>setShare({...share,recipientName:e.target.value})}/><Input type="email" placeholder="Recipient email" value={share.recipientEmail} onChange={e=>setShare({...share,recipientEmail:e.target.value})}/><Input type="number" min={1} max={30} value={share.expiresInDays} onChange={e=>setShare({...share,expiresInDays:Number(e.target.value)})}/><Button className={C.green} onClick={async()=>{const r=await json(`/api/courtyard/sales-intelligence/transitions/${t.id}/shares`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(share)});await navigator.clipboard.writeText(r.url);refresh();toast({title:"Secure link copied",description:r.url})}}>Create and copy link</Button></div>{(query.data.shares||[]).map((x:any)=><div key={x.id} className="flex justify-between rounded border bg-white p-2 text-sm"><span>{x.recipientName||x.recipientEmail||"Recipient"} · expires {new Date(x.expiresAt).toLocaleDateString()} · {x.accessCount} views{x.revokedAt?" · Revoked":""}</span>{!x.revokedAt&&<Button size="sm" variant="outline" onClick={()=>save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}/shares/${x.id}/revoke`,method:"PATCH"})}>Revoke</Button>}</div>)}</CardContent></Card>
  </div>;
}
