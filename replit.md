...
## System Architecture
The platform is built as a **monorepo** containing both web and mobile applications sharing a common backend. The structure includes:
- **Web Frontend** (`client/`): React application with Wouter routing, TanStack Query for data fetching, and shadcn/ui components
- **Mobile Frontend** (`mobile/`): React Native Expo application for iOS and Android with all website functionality except admin dashboard
- **Backend** (`server/`): Express.js application serving both web and mobile clients
- **Shared Types** (`shared/`): Common TypeScript schemas and types used by all applications
- **Database**: PostgreSQL via Drizzle ORM, accessed by backend
- **Authentication**: Replit Auth (OpenID Connect) with passport.js for both platforms
- **Real-time Messaging**: WebSocket server for both web and mobile clients
