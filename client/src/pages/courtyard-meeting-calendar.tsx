...
function MeetingSetupDiagram({ meetingRoom, roomSetup, attendance, banquetChairsPerTable = 8, orientation = "lengthwise", layout = [], onLayoutChange }: { meetingRoom: string; roomSetup: string; attendance: number | string; banquetChairsPerTable?: number | string; orientation?: string; layout?: any[]; onLayoutChange?: (layout: any[]) => void }) {
  ...
  const equipment=layout.filter((item:any)=>item.type&&!["presentation","entry","removed"].includes(item.type)).map((item:any)=>item.type==="small_table"&&item.chairs==null?{...item,chairs:2}:item);
      ...
      <Dialog open={open} onOpenChange={(isOpen) => { setOpen(isOpen); if (!isOpen) { setEditingEventId(null); setEventContractFile(null); } }}>
        <DialogContent className="max-h-[92dvh] max-w-5xl overflow-y-auto bg-white text-[#201814]">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            {(form.setupLayoutJson||[]).some((item:any)=>item.type==="small_table") && <div className="md:col-span-2 rounded-lg border border-[#deceba] bg-white p-3"><Label>Seats at added tables</Label><div className="mt-2 grid gap-2 sm:grid-cols-2">{(form.setupLayoutJson||[]).filter((item:any)=>item.type==="small_table").map((item:any)=><div key={`${item.id}-chairs`} className="flex items-center justify-between gap-3 rounded-md border p-2"><span className="truncate text-sm">{item.label||"Small table"}</span><Select value={String(item.chairs==null?2:item.chairs)} onValueChange={(value)=>setForm({...form,setupLayoutJson:form.setupLayoutJson.map((candidate:any)=>candidate.id===item.id?{...candidate,chairs:Number(value)}:candidate)})}><SelectTrigger className="w-28"><SelectValue /></SelectTrigger><SelectContent>{[0,1,2,3,4,5,6,7,8].map((count)=><SelectItem key={count} value={String(count)}>{count} seat{count===1?"":"s"}</SelectItem>)}</SelectContent></Select></div>)}</div></div>}
