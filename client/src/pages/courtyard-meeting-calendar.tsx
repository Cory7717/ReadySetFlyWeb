...
export default function CourtyardMeetingCalendar() {
  ...
  const importContract = useMutation<any, any, boolean>({
    ...
    onSuccess: (result: any) => { setContractOpen(false); setContractFile(null); setContractPreview(null); setContractMergeMessage(""); qc.invalidateQueries({ queryKey: ["meeting-calendar"] }); toast({ title: "Linked group booking imported", description: `Saved the room block and ${result.count} meeting-space dates.` }); },
