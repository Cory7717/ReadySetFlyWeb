...
function MeetingSetupDiagram({ meetingRoom, roomSetup, attendance, banquetChairsPerTable = 8, orientation = "lengthwise", layout = [], onLayoutChange }: { meetingRoom: string; roomSetup: string; attendance: number | string; banquetChairsPerTable?: number | string; orientation?: string; layout?: any[]; onLayoutChange?: (layout: any[]) => void }) {
  ...
  useEffect(()=>{if(!onLayoutChange||!selectedLayoutId)return;const removeSelected=(event:KeyboardEvent)=>{const target=event.target as HTMLElement|null;if(target?.closest("input, textarea, select, [contenteditable='true']"))return;if(event.key!=="Delete"&&event.key!=="Backspace")return;event.preventDefault();onLayoutChange(layout.filter((item:any)=>item.id!==selectedLayoutId));setSelectedLayoutId(null);};window.addEventListener("keydown",removeSelected);return()=>window.removeEventListener("keydown",removeSelected);},[layout,onLayoutChange,selectedLayoutId]);
  ...
  const equipment=layout.filter((item:any)=>item.type&&!["presentation","entry"].includes(item.type)).map((item:any)=>item.type==="small_table"&&item.chairs==null?{...item,chairs:2}:item);
  ...
  for(const item of equipment){
    ...
    if(item.id===selectedLayoutId)elements.push(<g key={`${item.id}-selected`} transform={`rotate(${rotation} ${p.x} ${p.y})`}><rect x={p.x-42} y={p.y-34} width="84" height="68" rx="6" fill="none" stroke="#dc2626" strokeWidth="2" strokeDasharray="6 4" pointerEvents="none"/><g className="cursor-pointer" role="button" aria-label={`Delete ${item.label||"setup item"}`} onPointerDown={(event:any)=>{event.preventDefault();event.stopPropagation();}} onClick={(event:any)=>{event.stopPropagation();onLayoutChange?.(layout.filter((candidate:any)=>candidate.id!==item.id));setSelectedLayoutId(null);}}><circle cx={p.x+42} cy={p.y-34} r="11" fill="#dc2626"/><text x={p.x+42} y={p.y-30} textAnchor="middle" fontSize="14" fontWeight="bold" fill="white">×</text></g></g>);
