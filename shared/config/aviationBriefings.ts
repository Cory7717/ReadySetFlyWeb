...
I understand that no payment is owed unless a separate written agreement is made. When reasonably possible, Ready Set Fly will provide the photo credit entered with this submission.

I confirm that the photograph does not knowingly violate another person's copyright, trademark, privacy, publicity, or other legal rights. If identifiable people appear in the photograph, I confirm that I have obtained any permission reasonably necessary to submit the image for editorial and promotional use.`;
...
export const AVIATION_BRIEFING_STATUSES = [
  "draft",
  "review",
  "scheduled",
  "published",
...
export const AVIATION_CONTRIBUTOR_ROLES = [
  "Author",
  "Host",
  "Presenter",
  "Guest",
...
export const AVIATION_SUBMISSION_STATUSES = [
  "draft",
  "submitted",
  "under_review",
  "revision_requested",
...
export const contributorSubmissionSchema = z
  .object({
    ...
    contributorTitle: z.string().trim().max(300).default(""),
    contributorCredentials: z.string().trim().max(500).default(""),
    contributorOrganization: z.string().trim().max(300).default(""),
    contributorBio: z.string().trim().max(3000).default(""),
    contributorProfileImageUrl: optionalHttpUrl,
    contributorWebsiteUrl: optionalHttpUrl,
    contributorYoutubeUrl: optionalHttpUrl,
    contributorVimeoUrl: optionalHttpUrl,
    contributorLinkedinUrl: optionalHttpUrl,
    contributorOtherUrl: optionalHttpUrl,
    title: z.string().trim().min(3).max(300),
    description: z.string().trim().min(10).max(2000),
    category: z.string().trim().min(2).max(120),
    videoProvider: z.enum(["youtube", "vimeo"]),
    videoUrl: optionalHttpUrl,
    originalPublicationUrl: optionalHttpUrl,
    ...
    videoDurationSeconds: z.coerce
      ...
      .nullable()
      .default(null),
    transcript: z.string().max(200000).default(""),
    contentOutline: z.string().max(30000).default(""),
    intendedAudience: z.string().max(2000).default(""),
    relevantToolIds: z.array(z.string().max(100)).max(30).default([]),
    additionalNotes: z.string().max(10000).default(""),
    isSponsored: z.boolean().default(false),
    sponsorName: z.string().max(500).default(""),
    hasAffiliateRelationship: z.boolean().default(false),
    affiliateDisclosure: z.string().max(3000).default(""),
    promotesProductsOrServices: z.boolean().default(false),
    receivedCompensation: z.boolean().default(false),
    commercialRelationshipNotes: z.string().max(5000).default(""),
    agreementAccepted: z.boolean().default(false),
  })
  .superRefine((input, context) => {
    const video = validateBriefingVideo({
      ...({} as AviationBriefingInput),
      contentType: "video",
      videoSourceType: input.videoProvider,
...
export type ContributorSubmissionInput = z.infer<
  typeof contributorSubmissionSchema
>;
...
export const aviationBriefingInputSchema = z.object({
  ...
  featuredImageCreditUrl: z
    .string()
    .trim()
    .max(2000)
    .refine(
      (value) => !value || /^https?:\/\//i.test(value),
      "Use a valid HTTP or HTTPS credit URL.",
    )
    .default(""),
...
export function validateBriefingVideo(input: AviationBriefingInput) {
  ...
  if (input.videoSourceType === "uploaded") {
    return input.videoStorageKey
      ? null
      : "Uploaded video storage is not configured for this briefing.";
  }
  try {
    const url = new URL(input.videoUrl);
    const host = url.hostname.replace(/^www\./, "").toLowerCase();
    if (
      input.videoSourceType === "youtube" &&
      !["youtube.com", "youtu.be"].includes(host)
    )
      return "Use a valid YouTube URL.";
    if (
      input.videoSourceType === "vimeo" &&
      !["vimeo.com", "player.vimeo.com"].includes(host)
    )
