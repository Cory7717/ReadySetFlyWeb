...
export const courtyardMeetingEvents = pgTable("courtyard_meeting_events", {
  ...
  serviceItemsJson: jsonb("service_items_json").$type<Array<{ name: string; serviceDates: string; chargeMethod: string; quantity: number; unitPrice: number; includedQuantity?: number; refillPrice?: number; instructions?: string }>>().notNull().default(sql`'[]'::jsonb`),
