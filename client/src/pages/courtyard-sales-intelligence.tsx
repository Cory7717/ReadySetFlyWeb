    ...
    <div className={`min-h-screen ${C.page}`}>
      ...
      <main className="sales-intelligence-content mx-auto max-w-7xl space-y-5 px-4 py-6">
        ...
        <section className="flex flex-wrap items-center gap-3 rounded-xl border border-[#cdbda8] bg-[#fffaf2] p-3 shadow-sm" aria-label="Sales planning tools">
          ...
          <Button className={view === "transition" ? C.green : C.outline} variant={view === "transition" ? "default" : "outline"} onClick={() => setView("transition")}>
            <FileClock className="mr-2 h-4 w-4" />
            Transition Hub
          </Button>
        ...
        <Tabs value={view} onValueChange={(value) => {
          ...
          <TabsContent value="transition">
            <SalesTransitionHub hotelId={selectedHotel} isAdmin={me.data.user.isAdmin} />
          </TabsContent>
          {marketSegments.map((segment: string) => (
...
function SalesTransitionHub({ hotelId, isAdmin }: { hotelId: string; isAdmin: boolean }) {
  const { toast } = useToast(); const qc = useQueryClient();
  const [setup, setSetup] = useState({ title: "Director of Sales Transition", departureDate: "", departingUserName: "", summary: "" });
  const emptyItem = { category: "system", title: "", description: "", status: "not_started", dueDate: "", ownerName: "", url: "", username: "", password: "", vaultUrl: "", mfaOwner: "", recoveryContact: "", frequency: "", confidential: false };
  const [item, setItem] = useState(emptyItem); const [file, setFile] = useState<File | null>(null); const [fileCategory, setFileCategory] = useState("other"); const [fileConfidential, setFileConfidential] = useState(false);
  const [share, setShare] = useState({ recipientName: "", recipientEmail: "", expiresInDays: 7, allowDownloads: true });
  const [editItem, setEditItem] = useState<any | null>(null); const [showPassword, setShowPassword] = useState(false);
  const query = useQuery({ queryKey: ["sales-transition", hotelId], queryFn: () => json(`/api/courtyard/sales-intelligence/transitions?hotelId=${encodeURIComponent(hotelId)}`), enabled: !!hotelId });
  const refresh = () => qc.invalidateQueries({ queryKey: ["sales-transition", hotelId] });
  const save = useMutation({ mutationFn: ({ url, method = "POST", body }: any) => json(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body || {}) }), onSuccess: () => { refresh(); toast({ title: "Transition workspace updated" }); }, onError: (e: Error) => toast({ title: "Update failed", description: e.message, variant: "destructive" }) });
  if (query.isLoading) return <Card className={C.shell}><CardContent className="p-6">Loading transition workspace…</CardContent></Card>;
  if (!query.data?.transition) return <Card className={C.shell}><CardHeader><CardTitle>Create Sales Transition Hub</CardTitle><CardDescription>One organized handoff for systems, accounts, opportunities, documents, contacts, and recurring work.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2"><Input placeholder="Title" value={setup.title} onChange={e => setSetup({...setup,title:e.target.value})}/><Input type="date" value={setup.departureDate} onChange={e => setSetup({...setup,departureDate:e.target.value})}/><Input placeholder="Departing DOS" value={setup.departingUserName} onChange={e => setSetup({...setup,departingUserName:e.target.value})}/><Input placeholder="Summary" value={setup.summary} onChange={e => setSetup({...setup,summary:e.target.value})}/><Button className={C.green} onClick={() => save.mutate({url:"/api/courtyard/sales-intelligence/transitions",body:{hotelId,...setup}})}>Create Transition Hub</Button></CardContent></Card>;
  const t = query.data.transition; const categories = ["system","account","opportunity","responsibility","contact","knowledge","deadline"];
  return <div className="space-y-4">
    <Card className={C.shell}><CardContent className="flex flex-wrap items-start justify-between gap-4 p-5"><div><div className="text-xs font-bold uppercase tracking-[.16em] text-[#8a6b3f]">Sales Transition Hub</div><h2 className="text-2xl font-semibold">{t.title}</h2><p className={C.muted}>{t.departingUserName || "Departing DOS"} · Last day {t.departureDate || "not set"}</p><p className="mt-2">{t.summary}</p></div><div className="rounded-lg border bg-white p-4 text-center"><div className="text-3xl font-bold text-[#2f5f46]">{query.data.progress}%</div><div className="mb-2 text-sm">complete</div><Button size="sm" variant="outline" disabled={!!t.departingSignedAt} onClick={() => save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}`,method:"PATCH",body:{action:"departing_signoff"}})}>{t.departingSignedAt ? "DOS signed off" : "DOS sign-off"}</Button>{isAdmin && <Button size="sm" className={`${C.green} ml-2`} disabled={!!t.managerAcceptedAt} onClick={() => save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}`,method:"PATCH",body:{action:"manager_accept"}})}>{t.managerAcceptedAt ? "Accepted" : "Manager accept"}</Button>}</div></CardContent></Card>
    <Card className={C.shell}><CardHeader><CardTitle>Add handoff item</CardTitle><CardDescription>Platform passwords are encrypted and never included in external share links. A password-manager vault link remains the preferred long-term option.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-3"><Select value={item.category} onValueChange={category=>setItem({...item,category})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{categories.map(x=><SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent></Select><Input placeholder="Title" value={item.title} onChange={e=>setItem({...item,title:e.target.value})}/><Input placeholder="New owner" value={item.ownerName} onChange={e=>setItem({...item,ownerName:e.target.value})}/><Textarea className="md:col-span-3" placeholder="Instructions, relationship context, next action, or knowledge" value={item.description} onChange={e=>setItem({...item,description:e.target.value})}/>{item.category==="system"&&<><Input placeholder="External URL" value={item.url} onChange={e=>setItem({...item,url:e.target.value})}/><Input placeholder="Login email/username" value={item.username} onChange={e=>setItem({...item,username:e.target.value})}/><Input type="password" autoComplete="new-password" placeholder="Platform password" value={item.password} onChange={e=>setItem({...item,password:e.target.value})}/><Input placeholder="Credential vault link" value={item.vaultUrl} onChange={e=>setItem({...item,vaultUrl:e.target.value})}/><Input placeholder="MFA owner" value={item.mfaOwner} onChange={e=>setItem({...item,mfaOwner:e.target.value})}/><Input placeholder="Recovery contact" value={item.recoveryContact} onChange={e=>setItem({...item,recoveryContact:e.target.value})}/></>}{item.category==="responsibility"&&<Input placeholder="Frequency" value={item.frequency} onChange={e=>setItem({...item,frequency:e.target.value})}/>}<Input type="date" value={item.dueDate} onChange={e=>setItem({...item,dueDate:e.target.value})}/><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={item.confidential} onChange={e=>setItem({...item,confidential:e.target.checked})}/>Confidential</label><Button className={C.green} disabled={!item.title.trim()} onClick={()=>{save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}/items`,body:item});setItem(emptyItem)}}><Plus className="mr-2 h-4 w-4"/>Add item</Button></CardContent></Card>
    <div className="grid gap-4 lg:grid-cols-2">{categories.map(category=>{const rows=(query.data.items||[]).filter((x:any)=>x.category===category);return rows.length?<Card className={C.shell} key={category}><CardHeader><CardTitle className="capitalize">{category}</CardTitle></CardHeader><CardContent className="space-y-2">{rows.map((x:any)=><div key={x.id} className="rounded border bg-white p-3"><div className="flex justify-between gap-3"><div><button className="text-left font-bold hover:text-[#2f5f46] hover:underline" onClick={()=>{setEditItem({...x,password:x.password||""});setShowPassword(false)}}>{x.title}</button>{x.confidential&&<Badge className="ml-2" variant="outline">Confidential</Badge>}<p className="text-sm text-[#5f5247]">{x.description}</p>{x.url&&<a className="text-sm text-blue-700 underline" href={x.url} target="_blank" rel="noreferrer">Open resource</a>}{x.username&&<div className="text-xs">Login: {x.username}</div>}{x.password&&<div className="text-xs font-semibold text-[#8a6b3f]">Password stored securely</div>}{x.vaultUrl&&<a className="text-xs text-blue-700 underline" href={x.vaultUrl} target="_blank" rel="noreferrer">Credential vault</a>}<div className="text-xs">{x.ownerName?`Owner: ${x.ownerName}`:"No owner"}{x.dueDate?` · Due ${x.dueDate}`:""}</div><Button className="mt-2" size="sm" variant="outline" onClick={()=>{setEditItem({...x,password:x.password||""});setShowPassword(false)}}>Open / Edit</Button></div><Select value={x.status} onValueChange={status=>save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}/items/${x.id}`,method:"PATCH",body:{status,ownerName:x.ownerName}})}><SelectTrigger className="w-36"><SelectValue/></SelectTrigger><SelectContent>{["not_started","in_progress","ready_for_review","complete","needs_follow_up"].map(s=><SelectItem key={s} value={s}>{s.replaceAll("_"," ")}</SelectItem>)}</SelectContent></Select></div></div>)}</CardContent></Card>:null})}</div>
    <Card className={C.shell}><CardHeader><CardTitle>Document archive</CardTitle><CardDescription>Contracts, reports, SOPs, templates, and collateral · 10 MB maximum. Confidential files are excluded from shared packages.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="flex flex-wrap items-center gap-2"><Input className="max-w-sm" type="file" onChange={e=>setFile(e.target.files?.[0]||null)}/><Input className="max-w-48" placeholder="Category" value={fileCategory} onChange={e=>setFileCategory(e.target.value)}/><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={fileConfidential} onChange={e=>setFileConfidential(e.target.checked)}/>Confidential</label><Button className={C.green} disabled={!file} onClick={async()=>{if(!file)return;try{const f=new FormData();f.append("file",file);f.append("category",fileCategory);f.append("confidential",String(fileConfidential));await json(`/api/courtyard/sales-intelligence/transitions/${t.id}/documents`,{method:"POST",body:f});setFile(null);setFileConfidential(false);refresh();toast({title:"Document archived"});}catch(error){toast({title:"Upload failed",description:(error as Error).message,variant:"destructive"});}}}>Archive file</Button></div>{(query.data.documents||[]).map((x:any)=><div key={x.id} className="flex justify-between rounded border bg-white p-2 text-sm"><span>{x.filename} · {x.category} · {Math.ceil(x.sizeBytes/1024)} KB{x.confidential?" · Confidential":""}</span><Button asChild size="sm" variant="outline"><a href={apiUrl(`/api/courtyard/sales-intelligence/transitions/${t.id}/documents/${x.id}`)}>Download</a></Button></div>)}</CardContent></Card>
    <Card className={C.shell}><CardHeader><CardTitle>Secure sharing</CardTitle><CardDescription>Links expire and can be revoked. Confidential and credential fields are always excluded.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="grid gap-2 md:grid-cols-4"><Input placeholder="Recipient name" value={share.recipientName} onChange={e=>setShare({...share,recipientName:e.target.value})}/><Input type="email" placeholder="Recipient email" value={share.recipientEmail} onChange={e=>setShare({...share,recipientEmail:e.target.value})}/><Input type="number" min={1} max={30} value={share.expiresInDays} onChange={e=>setShare({...share,expiresInDays:Number(e.target.value)})}/><Button className={C.green} onClick={async()=>{try{const r=await json(`/api/courtyard/sales-intelligence/transitions/${t.id}/shares`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(share)});await navigator.clipboard.writeText(r.url);refresh();toast({title:"Secure link copied",description:r.url});}catch(error){toast({title:"Could not create share link",description:(error as Error).message,variant:"destructive"});}}}>Create and copy link</Button></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={share.allowDownloads} onChange={e=>setShare({...share,allowDownloads:e.target.checked})}/>Allow downloads of non-confidential documents</label>{(query.data.shares||[]).map((x:any)=><div key={x.id} className="flex justify-between rounded border bg-white p-2 text-sm"><span>{x.recipientName||x.recipientEmail||"Recipient"} · expires {new Date(x.expiresAt).toLocaleDateString()} · {x.accessCount} views{x.allowDownloads?" · Downloads allowed":" · View only"}{x.revokedAt?" · Revoked":""}</span>{!x.revokedAt&&<Button size="sm" variant="outline" onClick={()=>save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}/shares/${x.id}/revoke`,method:"PATCH"})}>Revoke</Button>}</div>)}</CardContent></Card>
    <Dialog open={Boolean(editItem)} onOpenChange={open=>{if(!open)setEditItem(null)}}><DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto bg-white text-[#201814]"><DialogHeader><DialogTitle>Edit handoff item</DialogTitle><DialogDescription>Changes remain inside the authenticated Transition Hub. Passwords are encrypted and excluded from external shares.</DialogDescription></DialogHeader>{editItem&&<div className="grid gap-3 md:grid-cols-2"><Select value={editItem.category} onValueChange={category=>setEditItem({...editItem,category})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{categories.map(x=><SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent></Select><Select value={editItem.status} onValueChange={status=>setEditItem({...editItem,status})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{["not_started","in_progress","ready_for_review","complete","needs_follow_up"].map(s=><SelectItem key={s} value={s}>{s.replaceAll("_"," ")}</SelectItem>)}</SelectContent></Select><Input placeholder="Title" value={editItem.title||""} onChange={e=>setEditItem({...editItem,title:e.target.value})}/><Input placeholder="Owner" value={editItem.ownerName||""} onChange={e=>setEditItem({...editItem,ownerName:e.target.value})}/><Textarea className="md:col-span-2" placeholder="Instructions and handoff details" value={editItem.description||""} onChange={e=>setEditItem({...editItem,description:e.target.value})}/>{editItem.category==="system"&&<><Input placeholder="External URL" value={editItem.url||""} onChange={e=>setEditItem({...editItem,url:e.target.value})}/><Input placeholder="Login email/username" value={editItem.username||""} onChange={e=>setEditItem({...editItem,username:e.target.value})}/><div className="flex gap-2"><Input type={showPassword?"text":"password"} autoComplete="new-password" placeholder="Platform password" value={editItem.password||""} onChange={e=>setEditItem({...editItem,password:e.target.value})}/><Button type="button" variant="outline" onClick={()=>setShowPassword(!showPassword)}>{showPassword?"Hide":"Show"}</Button></div><Input placeholder="Credential vault link" value={editItem.vaultUrl||""} onChange={e=>setEditItem({...editItem,vaultUrl:e.target.value})}/><Input placeholder="MFA owner" value={editItem.mfaOwner||""} onChange={e=>setEditItem({...editItem,mfaOwner:e.target.value})}/><Input placeholder="Recovery contact" value={editItem.recoveryContact||""} onChange={e=>setEditItem({...editItem,recoveryContact:e.target.value})}/></>}{editItem.category==="responsibility"&&<Input placeholder="Frequency" value={editItem.frequency||""} onChange={e=>setEditItem({...editItem,frequency:e.target.value})}/>}<Input type="date" value={editItem.dueDate||""} onChange={e=>setEditItem({...editItem,dueDate:e.target.value})}/><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={Boolean(editItem.confidential)} onChange={e=>setEditItem({...editItem,confidential:e.target.checked})}/>Confidential</label><div className="flex justify-end gap-2 md:col-span-2"><Button variant="outline" onClick={()=>setEditItem(null)}>Cancel</Button><Button className={C.green} disabled={!String(editItem.title||"").trim()||save.isPending} onClick={()=>save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}/items/${editItem.id}`,method:"PATCH",body:editItem},{onSuccess:()=>setEditItem(null)})}>{save.isPending?"Saving…":"Save changes"}</Button></div></div>}</DialogContent></Dialog>
  </div>;
}
