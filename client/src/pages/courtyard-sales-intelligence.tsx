...
function SalesTransitionHub({ hotelId, isAdmin }: { hotelId: string; isAdmin: boolean }) {
  ...
  const emptyItem = { category: "system", title: "", description: "", status: "not_started", dueDate: "", ownerName: "", url: "", username: "", vaultUrl: "", mfaOwner: "", recoveryContact: "", frequency: "", confidential: false };
  ...
  return <div className="space-y-4">
    ...
    <Card className={C.shell}><CardHeader><CardTitle>Add handoff item</CardTitle><CardDescription>Never enter passwords or recovery codes. Add only the login name and approved password-manager vault link.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-3"><Select value={item.category} onValueChange={category=>setItem({...item,category})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{categories.map(x=><SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent></Select><Input placeholder="Title" value={item.title} onChange={e=>setItem({...item,title:e.target.value})}/><Input placeholder="New owner" value={item.ownerName} onChange={e=>setItem({...item,ownerName:e.target.value})}/><Textarea className="md:col-span-3" placeholder="Instructions, relationship context, next action, or knowledge" value={item.description} onChange={e=>setItem({...item,description:e.target.value})}/>{item.category==="system"&&<><Input placeholder="External URL" value={item.url} onChange={e=>setItem({...item,url:e.target.value})}/><Input placeholder="Login email/username" value={item.username} onChange={e=>setItem({...item,username:e.target.value})}/><Input placeholder="Credential vault link" value={item.vaultUrl} onChange={e=>setItem({...item,vaultUrl:e.target.value})}/><Input placeholder="MFA owner" value={item.mfaOwner} onChange={e=>setItem({...item,mfaOwner:e.target.value})}/><Input placeholder="Recovery contact" value={item.recoveryContact} onChange={e=>setItem({...item,recoveryContact:e.target.value})}/></>}{item.category==="responsibility"&&<Input placeholder="Frequency" value={item.frequency} onChange={e=>setItem({...item,frequency:e.target.value})}/>}<Input type="date" value={item.dueDate} onChange={e=>setItem({...item,dueDate:e.target.value})}/><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={item.confidential} onChange={e=>setItem({...item,confidential:e.target.checked})}/>Confidential</label><Button className={C.green} disabled={!item.title.trim()} onClick={()=>{save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}/items`,body:item});setItem(emptyItem)}}><Plus className="mr-2 h-4 w-4"/>Add item</Button></CardContent></Card>
    <div className="grid gap-4 lg:grid-cols-2">{categories.map(category=>{const rows=(query.data.items||[]).filter((x:any)=>x.category===category);return rows.length?<Card className={C.shell} key={category}><CardHeader><CardTitle className="capitalize">{category}</CardTitle></CardHeader><CardContent className="space-y-2">{rows.map((x:any)=><div key={x.id} className="rounded border bg-white p-3"><div className="flex justify-between gap-3"><div><b>{x.title}</b>{x.confidential&&<Badge className="ml-2" variant="outline">Confidential</Badge>}<p className="text-sm text-[#5f5247]">{x.description}</p>{x.url&&<a className="text-sm text-blue-700 underline" href={x.url} target="_blank" rel="noreferrer">Open resource</a>}{x.username&&<div className="text-xs">Login: {x.username}</div>}{x.vaultUrl&&<a className="text-xs text-blue-700 underline" href={x.vaultUrl} target="_blank" rel="noreferrer">Credential vault</a>}<div className="text-xs">{x.ownerName?`Owner: ${x.ownerName}`:"No owner"}{x.dueDate?` · Due ${x.dueDate}`:""}</div></div><Select value={x.status} onValueChange={status=>save.mutate({url:`/api/courtyard/sales-intelligence/transitions/${t.id}/items/${x.id}`,method:"PATCH",body:{status,ownerName:x.ownerName}})}><SelectTrigger className="w-36"><SelectValue/></SelectTrigger><SelectContent>{["not_started","in_progress","ready_for_review","complete","needs_follow_up"].map(s=><SelectItem key={s} value={s}>{s.replaceAll("_"," ")}</SelectItem>)}</SelectContent></Select></div></div>)}</CardContent></Card>:null})}</div>
