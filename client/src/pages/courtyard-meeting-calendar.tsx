...
import { COURTYARD_CATERING_TEMPLATES, cateringTemplateInstructions } from "@shared/courtyardCateringMenu";
...
const roomServiceFeeValue = (value: any) => roomRentalValue(value) * Number(value?.roomServiceFeePercent ?? 21) / 100;
const serviceItemGuestCount = (item: any, value: any) => Number(item?.quantity || 0) > 1 ? Number(item.quantity) : Number(value?.attendance || item?.quantity || 0);
const serviceItemAmount = (item: any, value: any) => item.chargeMethod === "complimentary" ? 0 : item.chargeMethod === "per_person" ? serviceItemGuestCount(item, value) * Number(item.unitPrice || 0) : item.chargeMethod === "per_person_per_day" ? serviceItemGuestCount(item, value) * eventDayCount(value) * Number(item.unitPrice || 0) : item.chargeMethod === "per_day" ? Number(item.quantity || 0) * eventDayCount(value) * Number(item.unitPrice || 0) : Number(item.quantity || 0) * Number(item.unitPrice || 0);
...
const eventRevenueTotal = (value: any) => roomRentalValue(value) + roomTaxValue(value) + roomServiceFeeValue(value) + fbSubtotal(value) + fbTaxValue(value) + fbGratuityValue(value) + Number(value?.avRevenue || 0);
const colors: any = {
...
const empty = {
  ...
  expectedRevenue: "",
  roomRentalRevenue: "",
  roomRentalChargeMethod: "per_event",
  avRevenue: "",
  cateringRevenue: "",
  breakfastPerPerson: "",
...
const emptyGroupRoom = {
  ...
  breakfastService: "none", breakfastGuaranteedCount: "", breakfastPricePerPerson: "", breakfastServiceDates: "", breakfastServiceTime: "", breakfastLocation: "",
...
const meetingMeals = (event: any): MealType[] => {
  const items = Array.isArray(event?.serviceItemsJson) ? event.serviceItemsJson : [];
  const hasBreakfastTemplate = items.some((item: any) => String(item?.menuCategory || "").toLowerCase() === "breakfast" || /\bbreakfast\b/i.test(String(item?.name || "")));
  const hasLunchTemplate = items.some((item: any) => String(item?.menuCategory || "").toLowerCase() === "lunch & dinner" || /\b(?:lunch|dinner|deli|taco buffet|fajita buffet|burger)\b/i.test(String(item?.name || "")));
  return [...(Number(event?.breakfastPerPerson || 0) > 0 || hasBreakfastTemplate ? ["breakfast" as const] : []), ...(Number(event?.lunchDinnerPerPerson || 0) > 0 || hasLunchTemplate ? ["lunch" as const] : [])];
};
function MealBadges({ meals }: { meals: MealType[] }) {
  if (!meals.length) return null;
  return <span className="inline-flex flex-wrap gap-1">{meals.map((meal) => <span key={meal} className={`inline-flex rounded-full border px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide ${meal === "breakfast" ? "border-amber-300 bg-amber-100 text-amber-900" : "border-emerald-300 bg-emerald-100 text-emerald-900"}`}>{meal === "breakfast" ? "Breakfast" : "Lunch"}</span>)}</span>;
}
function CalendarLegend() {
  return <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-[#4f443b]">{statusLegend.map(([label, swatch]) => <span key={label} className="inline-flex items-center gap-1.5"><span className={`h-3 w-3 rounded-sm border ${swatch}`} />{label}</span>)}<span className="mx-1 hidden h-4 border-l border-[#cdbda8] lg:block" /><BookingTypeBadge type="rooms" /><BookingTypeBadge type="event" /><BookingTypeBadge type="both" /><MealBadges meals={["breakfast", "lunch"]} /></div>;
}
...
function MeetingSetupDiagram({ meetingRoom, roomSetup, attendance, banquetChairsPerTable = 8, orientation = "lengthwise", layout = [], onLayoutChange }: { meetingRoom: string; roomSetup: string; attendance: number | string; banquetChairsPerTable?: number | string; orientation?: string; layout?: any[]; onLayoutChange?: (layout: any[]) => void }) {
  ...
  if (roomSetup === "banquet") {
    const seatsPerTable=Math.min(8,Math.max(1,Number(banquetChairsPerTable||8))),tables=Math.ceil(guests/seatsPerTable),cols=Math.ceil(Math.sqrt(tables*1.5));
    for (let i=0;i<tables;i++){const id=`table-${i}`;if(removedTableIds.has(id))continue;const p=position(id,105+(i%cols)*(390/Math.max(1,cols-1)),85+Math.floor(i/cols)*70),tableChairs=Math.min(seatsPerTable,guests-i*seatsPerTable),seatNodes=[] as any[];tablePositions.set(id,p);for(let seat=0;seat<tableChairs;seat++){const angle=-Math.PI/2+seat*(Math.PI*2/tableChairs);seatNodes.push(<circle key={seat} cx={p.x+Math.cos(angle)*(roundTableRadius+8)} cy={p.y+Math.sin(angle)*(roundTableRadius+8)} r="4" fill="#315f86"/>);}elements.push(<g key={i} {...drag(id)}><circle cx={p.x} cy={p.y} r={roundTableRadius} fill="#eadfce" stroke="#7b684f" strokeWidth="2"/>{seatNodes}<text x={p.x} y={p.y+4} textAnchor="middle" fontSize="9" fill="#201814">60&quot;</text></g>);}
  } else if (roomSetup === "classroom") {
    const tables=Math.ceil(guests/3),cols=orientation==="lengthwise"?Math.min(4,Math.max(1,Math.ceil(Math.sqrt(tables)))):Math.min(8,Math.max(1,Math.ceil(Math.sqrt(tables*1.7))));for(let i=0;i<tables;i++){const id=`table-${i}`;if(removedTableIds.has(id))continue;const p=position(id,orientation==="lengthwise"?180+(i%cols)*85:85+(i%cols)*60,orientation==="lengthwise"?95+Math.floor(i/cols)*78:90+Math.floor(i/cols)*50),tw=orientation==="lengthwise"?rectangleShort:rectangleLong,th=orientation==="lengthwise"?rectangleLong:rectangleShort;tablePositions.set(id,p);elements.push(<g key={i} {...drag(id)}><rect x={p.x-tw/2} y={p.y-th/2} width={tw} height={th} rx="2" fill="#eadfce" stroke="#7b684f"/>{orientation==="lengthwise"?<><circle cx={p.x-tw/2-7} cy={p.y-th*.3} r="3.5" fill="#315f86"/><circle cx={p.x-tw/2-7} cy={p.y} r="3.5" fill="#315f86"/><circle cx={p.x-tw/2-7} cy={p.y+th*.3} r="3.5" fill="#315f86"/></>:<><circle cx={p.x-tw*.3} cy={p.y+th/2+7} r="3.5" fill="#315f86"/><circle cx={p.x} cy={p.y+th/2+7} r="3.5" fill="#315f86"/><circle cx={p.x+tw*.3} cy={p.y+th/2+7} r="3.5" fill="#315f86"/></>}</g>);}
  } else if (roomSetup === "theater") {
    const cols=Math.min(12,Math.ceil(Math.sqrt(chairs*1.8)));for(let i=0;i<chairs;i++){elements.push(<circle key={i} cx={90+(i%cols)*(410/Math.max(1,cols-1))} cy={scaledY(80+Math.floor(i/cols)*24)} r="5" fill="#315f86"/>);}
  } else if (roomSetup === "u_shape") {
    const uChairs=chairs,bottomCount=Math.round(uChairs*.4),sideCount=uChairs-bottomCount,leftCount=Math.ceil(sideCount/2),rightCount=Math.floor(sideCount/2);
    elements.push(<path key="u" d={`M155 ${scaledY(90)} L155 ${scaledY(250)} L445 ${scaledY(250)} L445 ${scaledY(90)}`} fill="none" stroke="#7b684f" strokeWidth="20"/>);
    for(let i=0;i<leftCount;i++){const fraction=(i+1)/(leftCount+1);elements.push(<circle key={`left-${i}`} cx={132} cy={scaledY(90+fraction*160)} r="5" fill="#315f86"/>);}
    for(let i=0;i<rightCount;i++){const fraction=(i+1)/(rightCount+1);elements.push(<circle key={`right-${i}`} cx={468} cy={scaledY(90+fraction*160)} r="5" fill="#315f86"/>);}
    for(let i=0;i<bottomCount;i++){const fraction=(i+1)/(bottomCount+1);elements.push(<circle key={`bottom-${i}`} cx={155+fraction*290} cy={scaledY(273)} r="5" fill="#315f86"/>);}
    ...
    if(!removedTableIds.has("table-0")){tablePositions.set("table-0",p);elements.push(<g key="conference" {...drag("table-0")}><rect x={p.x-tw/2} y={p.y-th/2} width={tw} height={th} rx="10" fill="#eadfce" stroke="#7b684f" strokeWidth="2"/>{distributedChairs}</g>);}
  } else if (roomSetup === "reception") {
    const tables=Math.max(3,Math.ceil(guests/12));for(let i=0;i<tables;i++){const id=`table-${i}`;if(removedTableIds.has(id))continue;const p=position(id,105+(i%5)*98,95+Math.floor(i/5)*82);tablePositions.set(id,p);elements.push(<circle key={i} {...drag(id)} cx={p.x} cy={p.y} r="15" fill="#eadfce" stroke="#7b684f"/>);}
  }
  ...
  const equipment=layout.filter((item:any)=>item.type&&!["presentation","entry","removed"].includes(item.type)).map((item:any)=>item.type==="small_table"&&item.chairs==null?{...item,chairs:2}:item.type==="laptop_stand"?{...item,type:"podium"}:item);
  const equipmentSeats=(item:any,p:{x:number;y:number})=>{
    if(!["small_table","buffet_table"].includes(item.type))return null;
    if(item.type==="small_table"){const count=Number(item.chairs)===3?3:2;return Array.from({length:count},(_,index)=>{const fraction=(index+1)/(count+1);return <circle key={`side-${index}`} cx={p.x-rectangleLong/2+fraction*rectangleLong} cy={p.y+rectangleShort/2+8} r="4" fill="#315f86"/>;});}
    const count=Math.min(8,Math.max(0,Math.round(Number(item.chairs||0))));if(!count)return null;
    const nodes=[] as any[],endSeats=count>=4?2:0,sideSeats=count-endSeats,topCount=Math.ceil(sideSeats/2),bottomCount=Math.floor(sideSeats/2);
    for(let i=0;i<topCount;i++){const fraction=(i+1)/(topCount+1);nodes.push(<circle key={`top-${i}`} cx={p.x-rectangleLong/2+fraction*rectangleLong} cy={p.y-rectangleShort/2-8} r="4" fill="#315f86"/>);}
    for(let i=0;i<bottomCount;i++){const fraction=(i+1)/(bottomCount+1);nodes.push(<circle key={`bottom-${i}`} cx={p.x-rectangleLong/2+fraction*rectangleLong} cy={p.y+rectangleShort/2+8} r="4" fill="#315f86"/>);}
    if(endSeats){nodes.push(<circle key="end-left" cx={p.x-rectangleLong/2-8} cy={p.y} r="4" fill="#315f86"/>);nodes.push(<circle key="end-right" cx={p.x+rectangleLong/2+8} cy={p.y} r="4" fill="#315f86"/>);}
    return nodes;
  };
  const selectedTablePosition=selectedLayoutId?.startsWith("table-")?tablePositions.get(selectedLayoutId):null;
  if(selectedLayoutId&&selectedTablePosition)elements.push(<g key={`${selectedLayoutId}-selected`}><rect x={selectedTablePosition.x-42} y={selectedTablePosition.y-34} width="84" height="68" rx="6" fill="none" stroke="#dc2626" strokeWidth="2" strokeDasharray="6 4" pointerEvents="none"/><g className="cursor-pointer" role="button" aria-label="Delete selected table" onPointerDown={(event:any)=>{event.preventDefault();event.stopPropagation();}} onClick={(event:any)=>{event.stopPropagation();deleteLayoutItem(selectedLayoutId);}}><circle cx={selectedTablePosition.x+42} cy={selectedTablePosition.y-34} r="11" fill="#dc2626"/><text x={selectedTablePosition.x+42} y={selectedTablePosition.y-30} textAnchor="middle" fontSize="14" fontWeight="bold" fill="white">×</text></g></g>);
  for(const item of equipment){
    const p=position(item.id,300,175),rotation=Number(item.rotation||0);
    if(item.id===selectedLayoutId)elements.push(<g key={`${item.id}-selected`} transform={`rotate(${rotation} ${p.x} ${p.y})`}><rect x={p.x-42} y={p.y-34} width="84" height="68" rx="6" fill="none" stroke="#dc2626" strokeWidth="2" strokeDasharray="6 4" pointerEvents="none"/><g className="cursor-pointer" role="button" aria-label={`Delete ${item.label||"setup item"}`} onPointerDown={(event:any)=>{event.preventDefault();event.stopPropagation();}} onClick={(event:any)=>{event.stopPropagation();deleteLayoutItem(item.id);}}><circle cx={p.x+42} cy={p.y-34} r="11" fill="#dc2626"/><text x={p.x+42} y={p.y-30} textAnchor="middle" fontSize="14" fontWeight="bold" fill="white">×</text></g></g>);
    if(["small_table","buffet_table"].includes(item.type))elements.push(<g key={`${item.id}-chairs`} transform={`rotate(${rotation} ${p.x} ${p.y})`}>{equipmentSeats(item,p)}</g>);
  }
  const setupLabel=String(roomSetup||"custom").replaceAll("_"," ");
  const seatsPerTable=Math.min(8,Math.max(1,Number(banquetChairsPerTable||8))),equipmentSummary = roomSetup === "banquet" ? `${Math.ceil(guests/seatsPerTable)} rounds · ${seatsPerTable} chair${seatsPerTable===1?"":"s"} per table · 8 maximum` : roomSetup === "classroom" ? `${Math.ceil(guests/3)} classroom tables · up to 3 seats each` : roomSetup === "theater" ? `${guests} theater chairs` : roomSetup === "u_shape" ? `U-shape tables · ${guests} chairs` : roomSetup === "conference" ? `Conference table · ${guests} chairs` : roomSetup === "reception" ? `${Math.max(3,Math.ceil(guests/12))} cocktail tables` : "Equipment placement defined in setup notes";
  return <div className="rounded-xl border border-[#deceba] bg-[#fffaf2] p-4"><div className="mb-2 flex flex-wrap justify-between gap-2"><div><h3 className="font-semibold capitalize">{setupLabel} setup plan</h3><p className="text-xs text-[#5f5247]">{room.name} · approximately {room.widthFeet}' × {room.lengthFeet}' · {room.squareFeet.toLocaleString()} sq. ft. · {orientation}</p><p className="text-xs font-semibold text-[#315f86]">{equipmentSummary}</p></div><Badge variant="outline">{guests} guests</Badge></div><svg viewBox={`0 0 600 ${diagramHeight}`} className="w-full touch-none select-none rounded-lg bg-white" role="img" aria-label={`${setupLabel} overhead room setup for ${guests} guests`}><rect x="35" y="35" width={roomCanvasWidth} height={roomCanvasHeight} rx="4" fill="#faf8f4" stroke="#243746" strokeWidth="4"/>{elements}<g {...drag("presentation")} transform={`rotate(${presentationRotation} ${presentation.x} ${presentation.y})`}><rect x={presentation.x-62} y={presentation.y-13} width="124" height="26" rx="3" fill="#243746"/><text x={presentation.x} y={presentation.y+4} textAnchor="middle" fontSize="10" fill="white">PRESENTATION / FRONT</text></g><g {...drag("entry")} transform={`rotate(${entryRotation} ${entry.x} ${entry.y})`}><path d={`M${entry.x-15} ${entry.y-22} h30 v44`} fill="none" stroke="#2f5f46" strokeWidth="5"/><text x={entry.x+22} y={entry.y+4} fontSize="9" fill="#2f5f46">ENTRY / EXIT</text></g>{equipment.map((item:any)=>{const p=position(item.id,300,175),rotation=Number(item.rotation||0);return <g key={item.id} {...drag(item.id)} transform={`rotate(${rotation} ${p.x} ${p.y})`}>{item.type==="podium"?<path d={`M${p.x-14} ${p.y+16} L${p.x-10} ${p.y-16} L${p.x+10} ${p.y-16} L${p.x+14} ${p.y+16} Z`} fill="#a98254" stroke="#5f4025"/>:item.type==="tv"?<><rect x={p.x-27} y={p.y-20} width="54" height="32" rx="2" fill="#243746"/><line x1={p.x} y1={p.y+12} x2={p.x} y2={p.y+27} stroke="#5f5247" strokeWidth="3"/><line x1={p.x-15} y1={p.y+27} x2={p.x+15} y2={p.y+27} stroke="#5f5247" strokeWidth="3"/></>:<rect x={p.x-rectangleLong/2} y={p.y-rectangleShort/2} width={rectangleLong} height={rectangleShort} rx="3" fill="#eadfce" stroke="#7b684f"/>}<text x={p.x} y={p.y+(item.type==="tv"?-5:4)} textAnchor="middle" fontSize="8" fill={item.type==="tv"?"white":"#201814"}>{item.label}</text></g>})}{roomSetup === "custom"&&<text x="300" y="175" textAnchor="middle" fontSize="18" fill="#5f5247">Custom setup — refer to setup notes</text>}</svg><p className="mt-2 text-xs text-[#5f5247]">{onLayoutChange ? "Drag tables, equipment, presentation marker, and entry/exit to place them manually. " : ""}Table sizes are drawn to room scale (60-inch rounds and 72 × 30-inch rectangles). Confirm measurements, accessibility, fire-code capacity, and unobstructed exits onsite before setup.</p></div>;
}
...
export default function CourtyardMeetingCalendar() {
  ...
  const importContract = useMutation<any, any, boolean>({
    ...
    onSuccess: (result: any) => { const importedDate = contractPreview?.draft?.groupRoom?.arrivalDate || contractPreview?.draft?.meeting?.eventDate; if (importedDate) { const [year, monthNumber] = String(importedDate).split("-").map(Number); if (year && monthNumber) setMonth(new Date(year, monthNumber - 1, 1)); } const eventOnly = !contractPreview?.draft?.groupRoom && Boolean(result.count); setCalendarLayer(eventOnly ? "meetings" : result.count ? "all" : "groups"); setView("month"); setContractOpen(false); setContractFile(null); setContractPreview(null); setContractMergeMessage(""); qc.invalidateQueries({ queryKey: ["meeting-calendar"] }); toast({ title: eventOnly ? "Meeting/Event imported" : result.count ? "Linked booking imported" : "Group Rooms booking imported", description: eventOnly ? `Created ${result.count} meeting-space date${result.count === 1 ? "" : "s"} and stored the signed contract.` : result.count ? `Saved the room block and ${result.count} meeting-space date${result.count === 1 ? "" : "s"}. The calendar was moved to the imported dates.` : "Saved the rooms-only contract and moved the calendar to its arrival month." }); },
  ...
  const groupMeals = (block: any): MealType[] => {
    const linkedMeals = block.groupBookingId ? events.filter((event: any) => event.groupBookingId === block.groupBookingId).flatMap(meetingMeals) : [];
    return [...(block.breakfastService && block.breakfastService !== "none" ? ["breakfast" as const] : []), ...(linkedMeals.includes("breakfast") ? ["breakfast" as const] : []), ...(linkedMeals.includes("lunch") ? ["lunch" as const] : [])].filter((meal, index, all) => all.indexOf(meal) === index);
  };
  ...
  const openEdit = (event: any) => {
    ...
    setForm({
      ...
      roomRentalChargeMethod: event.roomRentalChargeMethod || "per_event",
  ...
  return (
    <div className="min-h-screen bg-[#f7f1e7] text-[#201814]">
                  ...
                  >
                    ...
                    {rows.map((x: any) => (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-1"><span>{x.status.replaceAll("_", " ")}</span><MealBadges meals={meetingMeals(x)} /></div>
                      </div>
                    ))}
                    {groupRows.map((block: any) => {
                      const marker = date === block.arrivalDate ? "ARRIVAL" : date === block.departureDate ? "DEPARTURE" : "IN HOUSE";
                      return <div key={block.id} className={`mt-1 cursor-pointer rounded border border-blue-200 p-1 text-xs hover:ring-2 hover:ring-[#315f86] ${groupStatusColors[block.status] || groupStatusColors.prospect}`} onClick={(event) => { event.stopPropagation(); setSelectedGroupRoom(block); }}><div className="flex items-center gap-1"><span className="min-w-0 flex-1 truncate"><b>{marker}</b> · {block.groupName}</span><BookingTypeBadge type={groupBookingType(block)} /></div><div className="flex flex-wrap items-center justify-between gap-1"><span>{date === block.departureDate ? "Checks out" : `${block.peakRooms || 0} rooms`}</span><MealBadges meals={groupMeals(block)} /></div></div>;
                    })}
                  </button>
                );
              })}
            ...
            <CardContent className="space-y-2 p-4">
              {calendarLayer !== "groups" && events.filter((event: any) => dateInDisplayedMonth(event.eventDate)).map((x: any) => (
                ...
                >
                  <div>
                    <b>
                      {x.eventDate} · {x.groupName} — {x.eventName} <BookingTypeBadge type={meetingBookingType(x)} /> <MealBadges meals={meetingMeals(x)} />
                    </b>
      ...
      <Dialog open={open} onOpenChange={(isOpen) => { setOpen(isOpen); if (!isOpen) { setEditingEventId(null); setEventContractFile(null); } }}>
        <DialogContent className="max-h-[92dvh] max-w-5xl overflow-y-auto bg-white text-[#201814]">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            {form.roomSetup === "banquet" && <div><Label>Chairs per 60-inch round</Label><Select value={String(form.banquetChairsPerTable || 8)} onValueChange={(value)=>setForm({...form,banquetChairsPerTable:Number(value),setupLayoutJson:(form.setupLayoutJson||[]).filter((item:any)=>!String(item.id).startsWith("table-"))})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{[1,2,3,4,5,6,7,8].map((count)=><SelectItem key={count} value={String(count)}>{count} chair{count===1?"":"s"}</SelectItem>)}</SelectContent></Select><p className="mt-1 text-xs text-[#5f5247]">Maximum eight chairs per table. The last table uses only the remaining chairs needed.</p></div>}
            ...
            {(form.setupLayoutJson||[]).some((item:any)=>item.type==="small_table") && <div className="md:col-span-2 rounded-lg border border-[#deceba] bg-white p-3"><Label>Seats at added tables</Label><p className="text-xs text-[#5f5247]">Small tables may seat two or three guests along one side.</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{(form.setupLayoutJson||[]).filter((item:any)=>item.type==="small_table").map((item:any)=><div key={`${item.id}-chairs`} className="flex items-center justify-between gap-3 rounded-md border p-2"><span className="truncate text-sm">{item.label||"Small table"}</span><Select value={String(Number(item.chairs)===3?3:2)} onValueChange={(value)=>setForm({...form,setupLayoutJson:form.setupLayoutJson.map((candidate:any)=>candidate.id===item.id?{...candidate,chairs:Number(value)}:candidate)})}><SelectTrigger className="w-28"><SelectValue /></SelectTrigger><SelectContent>{[2,3].map((count)=><SelectItem key={count} value={String(count)}>{count} seats</SelectItem>)}</SelectContent></Select></div>)}</div></div>}
            <div className="md:col-span-2"><Button type="button" size="sm" variant="outline" onClick={()=>setForm({...form,setupLayoutJson:[...(form.setupLayoutJson||[]),{id:`equipment-${Date.now()}-laptop-stand`,type:"laptop_stand",label:"A/V stand",x:.5,y:.5,rotation:0}]})}><Plus className="mr-1 h-3.5 w-3.5" />Projector / laptop stand</Button></div>
            <div className="md:col-span-2"><MeetingSetupDiagram meetingRoom={form.meetingRoom} roomSetup={form.roomSetup} attendance={form.attendance} banquetChairsPerTable={form.banquetChairsPerTable} orientation={form.setupOrientation} layout={form.setupLayoutJson||[]} onLayoutChange={(setupLayoutJson)=>setForm((current:any)=>({...current,setupLayoutJson}))} /></div>
            <Input
            ...
            <div className="md:col-span-2 rounded-xl border border-[#deceba] bg-[#fffaf2] p-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <div><Label>Room rental amount</Label><Input type="number" min="0" step="0.01" value={form.roomRentalRevenue} onChange={(event) => setForm({ ...form, roomRentalRevenue: event.target.value })} /><p className="mt-1 text-xs text-[#5f5247]">Calculated rental: {money(roomRentalValue(form))}</p></div>
        ...
        <DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto bg-white text-[#201814]">
          {selectedEvent && (
            <>
              ...
              <MeetingSetupDiagram meetingRoom={selectedEvent.meetingRoom} roomSetup={selectedEvent.roomSetup} attendance={selectedEvent.attendance} banquetChairsPerTable={selectedEvent.banquetChairsPerTable} orientation={selectedEvent.setupOrientation} layout={selectedEvent.setupLayoutJson||[]} />
              ...
              {selectedEvent.groupBookingId && (cal.data?.groupBookingDocuments || []).some((document: any) => document.groupBookingId === selectedEvent.groupBookingId) && <section><h3 className="mb-2 font-semibold">Original contract</h3>{(cal.data?.groupBookingDocuments || []).filter((document: any) => document.groupBookingId === selectedEvent.groupBookingId).map((document: any) => <a key={document.id} className="block rounded-lg border p-3 text-blue-700 underline hover:bg-[#f4f8fb]" href={apiUrl(`/api/courtyard/sales-intelligence/meeting-calendar/group-bookings/${selectedEvent.groupBookingId}/documents/${document.id}`)}>{document.filename}</a>)}</section>}
      ...
      <Dialog open={groupRoomOpen} onOpenChange={(isOpen) => { setGroupRoomOpen(isOpen); if (!isOpen) { setEditingGroupRoomId(null); setGroupContractFile(null); } }}>
        <DialogContent className="max-h-[92dvh] max-w-4xl overflow-y-auto border-[#cdbda8] bg-white text-[#201814] dark:border-[#cdbda8] dark:bg-white dark:text-[#201814] [&_input]:!border-[#cdbda8] [&_input]:!bg-white [&_input]:!text-[#201814] [&_textarea]:!border-[#cdbda8] [&_textarea]:!bg-white [&_textarea]:!text-[#201814]">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2 rounded-xl border border-[#d8c79e] bg-[#fffaf2] p-4"><div><h3 className="font-semibold text-[#6f4c18]">Contracted breakfast service</h3><p className="text-xs text-[#5f5247]">These details appear prominently on the printable Group Resume for Bistro and operations.</p></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><div><Label>Breakfast arrangement</Label><Select value={groupRoomForm.breakfastService||"none"} onValueChange={(breakfastService)=>setGroupRoomForm({...groupRoomForm,breakfastService})}><SelectTrigger className="bg-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">No contracted breakfast</SelectItem><SelectItem value="included">Breakfast included</SelectItem><SelectItem value="contracted_buffet">Contracted breakfast buffet</SelectItem></SelectContent></Select></div><div><Label>Guaranteed guests</Label><Input type="number" min="0" step="1" value={groupRoomForm.breakfastGuaranteedCount} onChange={(e)=>setGroupRoomForm({...groupRoomForm,breakfastGuaranteedCount:e.target.value})}/></div><div><Label>Price per person</Label><Input type="number" min="0" step="0.01" disabled={groupRoomForm.breakfastService==="included"} value={groupRoomForm.breakfastPricePerPerson} onChange={(e)=>setGroupRoomForm({...groupRoomForm,breakfastPricePerPerson:e.target.value})}/></div><div><Label>Service dates</Label><Input placeholder="Daily during stay or specific dates" value={groupRoomForm.breakfastServiceDates} onChange={(e)=>setGroupRoomForm({...groupRoomForm,breakfastServiceDates:e.target.value})}/></div><div><Label>Service time</Label><Input placeholder="6:30 AM–9:30 AM" value={groupRoomForm.breakfastServiceTime} onChange={(e)=>setGroupRoomForm({...groupRoomForm,breakfastServiceTime:e.target.value})}/></div><div><Label>Location</Label><Input placeholder="Bistro or meeting room" value={groupRoomForm.breakfastLocation} onChange={(e)=>setGroupRoomForm({...groupRoomForm,breakfastLocation:e.target.value})}/></div></div></div>
      ...
      <Dialog open={Boolean(selectedGroupRoom)} onOpenChange={(isOpen) => { if (!isOpen) setSelectedGroupRoom(null); }}>
        <DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto bg-white text-[#201814]">
          {selectedGroupRoom && <><DialogHeader><div className="flex items-start justify-between gap-3 pr-7"><div><div className="flex flex-wrap items-center gap-2"><DialogTitle className="text-2xl">{selectedGroupRoom.groupName}</DialogTitle><BookingTypeBadge type={groupBookingType(selectedGroupRoom)} /></div><p className="text-[#5f5247]">{selectedGroupRoom.projectName || "Group room block"}</p></div><Badge className={groupStatusColors[selectedGroupRoom.status]}>{selectedGroupRoom.status.replaceAll("_", " ")}</Badge></div><div className="flex flex-wrap gap-2 pt-2"><Button asChild size="sm" className="bg-[#315f86] text-white hover:bg-[#244966]"><a href={apiUrl(`/api/courtyard/sales-intelligence/meeting-calendar/group-rooms/${selectedGroupRoom.id}/resume.pdf`)} target="_blank" rel="noreferrer"><Printer className="mr-2 h-4 w-4" />View / print Group Resume</a></Button><Button size="sm" variant="outline" className="border-red-300 text-red-700 hover:bg-red-50 hover:text-red-800" disabled={deleteGroupRoom.isPending} onClick={() => confirmDeleteGroup(selectedGroupRoom)}><Trash2 className="mr-2 h-4 w-4" />Delete group</Button><Button size="sm" variant="outline" className="border-[#bfd0df] text-[#315f86] hover:bg-[#f4f8fb]" disabled={linkMatchingGroup.isPending} onClick={() => linkMatchingGroup.mutate(selectedGroupRoom)}>{linkMatchingGroup.isPending ? "Linking…" : "Link matching entries"}</Button></div></DialogHeader>
            ...
            {(selectedGroupRoom.breakfastService&&selectedGroupRoom.breakfastService!=="none"||selectedGroupRoom.breakfastNotes)&&<section className="rounded-xl border border-[#d8c79e] bg-[#fffaf2] p-4"><h3 className="font-semibold text-[#6f4c18]">Breakfast service</h3><div className="mt-2 grid gap-2 sm:grid-cols-2"><div><b>Arrangement:</b> {selectedGroupRoom.breakfastService==="contracted_buffet"?"Contracted breakfast buffet":selectedGroupRoom.breakfastService==="included"?"Breakfast included":"Breakfast details"}</div><div><b>Guaranteed guests:</b> {selectedGroupRoom.breakfastGuaranteedCount??"Not specified"}</div><div><b>Dates/time:</b> {selectedGroupRoom.breakfastServiceDates||"During group stay"} · {selectedGroupRoom.breakfastServiceTime||"Time not specified"}</div><div><b>Location:</b> {selectedGroupRoom.breakfastLocation||"Not specified"}</div><div><b>Price:</b> {selectedGroupRoom.breakfastService==="included"?"Included":`${money(selectedGroupRoom.breakfastPricePerPerson)} per person`}</div></div>{selectedGroupRoom.breakfastNotes&&<p className="mt-3 whitespace-pre-wrap text-sm">{selectedGroupRoom.breakfastNotes}</p>}</section>}
      ...
      <Dialog open={contractOpen} onOpenChange={(isOpen) => { setContractOpen(isOpen); if (!isOpen) { setContractFile(null); setContractPreview(null); setContractMergeMessage(""); } }}>
        <DialogContent className="max-h-[92dvh] max-w-4xl overflow-y-auto border-[#cdbda8] bg-white text-[#201814] dark:border-[#cdbda8] dark:bg-white dark:text-[#201814] [&_input]:!border-[#cdbda8] [&_input]:!bg-white [&_input]:!text-[#201814] [&_textarea]:!border-[#cdbda8] [&_textarea]:!bg-white [&_textarea]:!text-[#201814]">
          <DialogHeader><DialogTitle>Import contract</DialogTitle></DialogHeader>
          {!contractPreview ? <div className="space-y-4"><div className="rounded-xl border-2 border-dashed border-[#bfd0df] bg-[#f4f8fb] p-8 text-center"><Upload className="mx-auto mb-3 h-9 w-9 text-[#315f86]" /><Label htmlFor="group-contract" className="text-base font-semibold">Choose a DOCX or PDF contract</Label><Input id="group-contract" className="mx-auto mt-3 max-w-lg bg-white" type="file" accept=".docx,.pdf,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => setContractFile(event.target.files?.[0] || null)} /><p className="mt-2 text-sm text-[#5f5247]">DOCX provides the most reliable table extraction. The original file will be stored with the linked group booking.</p></div><Button className="w-full bg-[#315f86] text-white" disabled={!contractFile || previewContract.isPending} onClick={() => previewContract.mutate()}>{previewContract.isPending ? "Reading contract…" : "Review extracted information"}</Button></div> : <div className="space-y-5">
            ...
            {contractPreview.draft.groupRoom && <section><h3 className="mb-3 flex items-center gap-2 font-semibold"><BedDouble className="h-5 w-5 text-[#315f86]" />Group Rooms</h3><div className="grid gap-3 md:grid-cols-3">
            ...
            {contractPreview.draft.meeting && <section><h3 className="mb-3 flex items-center gap-2 font-semibold"><CalendarDays className="h-5 w-5 text-[#2f5f46]" />{contractPreview.draft.groupRoom ? "Linked Meeting Space" : "Meeting/Event"}</h3><div className="grid gap-3 md:grid-cols-3">
              {[['Group/company','groupName','text'],['Event name','eventName','text'],['Start date','eventDate','date'],['End date','eventEndDate','date'],['Attendance','attendance','number'],['Room rental rate','roomRentalRevenue','number'],['Breakfast/person/day','breakfastPerPerson','number'],['Snack bar/person/day','snackBarPerPerson','number'],['Room tax %','roomTaxPercent','number'],['Room service fee %','roomServiceFeePercent','number'],['F&B tax %','fbTaxPercent','number'],['F&B gratuity %','fbGratuityPercent','number'],['Client contact','clientName','text'],['Client email','clientEmail','email'],['Client phone','clientPhone','text'],['Sales owner','salesOwner','text'],['Setup begins','setupStartTime','time'],['Guests begin','guestStartTime','time'],['Guests end','guestEndTime','time'],['Breakdown complete','breakdownEndTime','time']].map(([label,field,type]) => <div key={field}><Label>{label}</Label><Input type={type} value={contractPreview.draft.meeting[field] ?? ''} onChange={(event) => setContractPreview({ ...contractPreview, draft: { ...contractPreview.draft, meeting: { ...contractPreview.draft.meeting, [field]: event.target.value } } })} /></div>)}
            ...
            </div>{Array.isArray(contractPreview.draft.meeting.serviceItemsJson) && contractPreview.draft.meeting.serviceItemsJson.length > 0 && <div className="mt-4 rounded-xl border border-[#deceba] bg-[#fffaf2] p-4"><h4 className="font-semibold">Detected catering selections</h4><p className="mb-3 text-xs text-[#5f5247]">Confirm the negotiated price, service dates, and inclusions before importing. These details will print on the BEO.</p><div className="space-y-3">{contractPreview.draft.meeting.serviceItemsJson.map((item:any,index:number)=><div key={`${item.name}-${index}`} className="rounded-lg border border-[#deceba] bg-white p-3"><div className="grid gap-2 md:grid-cols-6"><div className="md:col-span-2"><Label>Menu selection</Label><Input value={item.name} onChange={(event)=>{const rows=[...contractPreview.draft.meeting.serviceItemsJson];rows[index]={...item,name:event.target.value};setContractPreview({...contractPreview,draft:{...contractPreview.draft,meeting:{...contractPreview.draft.meeting,serviceItemsJson:rows}}})}} /></div><div><Label>Price</Label><Input type="number" min="0" step="0.01" value={item.unitPrice} onChange={(event)=>{const rows=[...contractPreview.draft.meeting.serviceItemsJson];rows[index]={...item,unitPrice:event.target.value};setContractPreview({...contractPreview,draft:{...contractPreview.draft,meeting:{...contractPreview.draft.meeting,serviceItemsJson:rows}}})}} /></div><div><Label>Charge method</Label><Select value={item.chargeMethod} onValueChange={(chargeMethod)=>{const rows=[...contractPreview.draft.meeting.serviceItemsJson];rows[index]={...item,chargeMethod};setContractPreview({...contractPreview,draft:{...contractPreview.draft,meeting:{...contractPreview.draft.meeting,serviceItemsJson:rows}}})}}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent className="bg-white text-[#201814]"><SelectItem value="per_person">Per person</SelectItem><SelectItem value="per_person_per_day">Per person/day</SelectItem><SelectItem value="per_event">Per event</SelectItem><SelectItem value="per_day">Per day</SelectItem><SelectItem value="complimentary">Complimentary</SelectItem></SelectContent></Select></div><div className="md:col-span-2"><Label>Service dates</Label><Input value={item.serviceDates} onChange={(event)=>{const rows=[...contractPreview.draft.meeting.serviceItemsJson];rows[index]={...item,serviceDates:event.target.value};setContractPreview({...contractPreview,draft:{...contractPreview.draft,meeting:{...contractPreview.draft.meeting,serviceItemsJson:rows}}})}} /></div><div className="md:col-span-6"><Label>Included items / instructions</Label><Textarea value={item.instructions} onChange={(event)=>{const rows=[...contractPreview.draft.meeting.serviceItemsJson];rows[index]={...item,instructions:event.target.value};setContractPreview({...contractPreview,draft:{...contractPreview.draft,meeting:{...contractPreview.draft.meeting,serviceItemsJson:rows}}})}} /></div></div></div>)}</div></div>}</section>}
            {contractPreview.draft.groupRoom && <div className="rounded-lg border bg-[#fffaf2] p-3 text-sm"><b>Revenue allocation:</b> The lodging rate is kept separate from breakfast and meeting-space revenue so a packaged rate is not counted twice.</div>}
